...
const AirplaneIcon = forwardRef<AnimatedIconHandle, AnimatedIconProps>(
  (
    ...
    const start = useCallback(async () => {
      ...
      animate(".wind1", { x: -12, y: 24, opacity: 1 }, { duration: 0.8, ease: "easeOut" });
      animate(".wind2", { x: 0, y: 26, opacity: 1 }, { duration: 0.8, ease: "easeOut", delay: 0.2 });
    }, [animate]);
    ...
    return (
      ...
      >
        ...
        />
        <motion.path className="wind1" initial={{ opacity: 0, x: 10, y: -6 }} d="m.5 3.5 3-3" />
        <motion.path className="wind2" initial={{ opacity: 0, x: 26, y: 8 }} d="m.5 3.5 3-3" />
      </motion.svg>
    );
  },
);
