/**
 * 建立「澳洲 總量型 eSIM」
 *   OPTUS ← Australia-Total*GB-*-A0/A1（SG IP、e-ideas、高速後 128kbps、110%）
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-australia-total-product.mjs
 *   HKD_TO_TWD=4.5 node scripts/create-australia-total-product.mjs --rebuild
 */
import fs from "fs";
...
const HANDLE = "australia-total-esim";
const TITLE = "澳洲 總量型 eSIM";
...
function collectRows(raw) {
  ...
  for (const p of raw) {
    ...
    if (!/^Australia-Total\d+GB-\d+-A[01]$/i.test(name)) continue;
...
async function main() {
  ...
  console.log(`  ${TELECOM} ← Australia-Total*-A0/A1（128kbps）`);

  const rows = collectRows(await fetchPlans());
  if (!rows.length) throw new Error("找不到 Australia-Total*GB-*-A0/A1");

  const samples = [
    rows.find((r) => r.day === 3 && r.data === "1GB"),
  ...
  const productMeta = {
    ...
    seo_title: "澳洲 eSIM 總量型｜Optus 5G｜Jeko eSIM",
    seo_description:
      "澳洲總量型 eSIM，Optus 4G／5G。可選 1GB～50GB，高速用完後約 128kbps。新加坡 IP，支援 ChatGPT、TikTok、Gemini。",
    seo_keywords:
      "澳洲eSIM,Australia eSIM,Optus,總量型,旅遊eSIM,Jeko eSIM",
    subtitle_by_carrier: {
      [TELECOM]: "Optus 4G／5G｜總量高速後約 128kbps｜新加坡 IP",
    },
    ...
    overview_notices_by_carrier: {
      [TELECOM]: {
        fup_notice:
          "Optus 漫遊總量型。高速用完後降速約 128kbps。出網新加坡 IP，支援 ChatGPT／TikTok／Gemini。偏遠內陸收訊不保證。",
  ...
  const payloadBase = {
    title: TITLE,
    subtitle: "Optus 4G／5G｜總量型｜高速後約 128kbps｜110%",
    handle: HANDLE,
    description:
      "澳洲總量型 eSIM，走 Optus 4G／5G。可選 1GB／3GB／5GB／10GB／20GB／30GB／50GB，3～30 天。高速用完後約 128kbps 可持續使用。新加坡 IP（APN e-ideas），支援 ChatGPT、TikTok、Gemini 與熱點。建議抵達澳洲後再啟用。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  if (!product) {
    ...
    if (!REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-australia-total-product.mjs --rebuild",
      );
      return;
    }
