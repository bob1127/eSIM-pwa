/**
 * 建立「美加 (美國、加拿大) 總量型 eSIM」
 * SKU：US,CA-Total*GB-*-A0、US&Canada-Total*GB-*-B0/B1
 * 利潤：70%
 * 不含墨西哥；總量用完斷網（rule=terminate）
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-us-canada-total-product.mjs
 *   HKD_TO_TWD=4.5 node scripts/create-us-canada-total-product.mjs --rebuild
 */
import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";
import { usCanadaTotalKeyFeaturesByCarrier } from "../content/product-detailed/usa-region-key-features.js";
...
const HANDLE = "us-canada-total-esim";
const TITLE = "美加 (美國、加拿大) 總量型 eSIM";
const LINE = "漫遊線路";
const PROFIT = 70;
...
const THUMB = process.env.US_ESIM_PRODUCT_THUMB || DEFAULT_US_THUMB;
...
function loadPlans() {
  const file = path.join(__dirname, "data", "us-canada-total-plans.json");
  ...
  return (raw.plans || [])
    ...
    .sort(
      (a, b) =>
        a.day - b.day ||
        String(a.data).localeCompare(String(b.data), "zh-TW") ||
...
function toVariant(row) {
  ...
  const data = row.data;
  const isTerminate = /terminate|stop/i.test(String(row.rule_desc || ""));
  const speedRule = isTerminate
    ? `${data}；用完斷網`
    : `${data}；降速後無限（FUP）`;
  return {
...
async function main() {
  ...
  if (!rows.length) throw new Error("us-canada-total-plans.json 無資料");

  console.log(`💱 HKD→TWD ${HKD_TO_TWD}｜利潤 ${PROFIT}%｜方案 ${rows.length}`);
  for (const s of rows.filter((r) => r.day === 3 && /3GB|5GB/.test(r.data))) {
    console.log(
      `   ${s.sku} → cost NT$${s.cost_twd} → 售價 NT$${s.retail_twd}｜${s.data}`,
    );
  }
  ...
  const dataValues = [...new Set(rows.map((r) => r.data))].sort((a, b) => {
    const na = Number(String(a).match(/(\d+(?:\.\d+)?)/)?.[1] || 0);
    const nb = Number(String(b).match(/(\d+(?:\.\d+)?)/)?.[1] || 0);
    return na - nb;
  });
  ...
  const productMeta = {
    ...
    seo_title: "美加 eSIM 總量型｜美國加拿大｜Jeko eSIM",
    seo_description:
      "美加（美國、加拿大）總量型 eSIM，純數據，不含墨西哥。多組流量與天數可選。",
    seo_keywords: "美加eSIM,美國加拿大eSIM,總量型,Jeko eSIM",
    subtitle_by_carrier: Object.fromEntries(
      telecomValues.map((t) => [t, `美加覆蓋｜${t}｜總量型`]),
    ),
    ...
    overview_notices_by_carrier: Object.fromEntries(
      telecomValues.map((t) => [
        ...
        {
          fup_notice:
            "總量型方案：高速額度用完後斷網（非降速吃到飽）。僅涵蓋美國與加拿大，不含墨西哥。",
          activation_notice: "建議抵達後再新增／啟用 eSIM。",
        },
      ]),
    ),
    key_features_by_carrier: usCanadaTotalKeyFeaturesByCarrier(),
  };

  const payloadBase = {
    title: TITLE,
    subtitle: "美國 + 加拿大｜總量型｜純數據｜不含墨西哥｜利潤 70%",
    handle: HANDLE,
    description:
      "美加（美國、加拿大）總量型 eSIM。純數據方案，覆蓋美加兩國，不含墨西哥。提供 1–50GB、3–30 天多組方案。總量用完斷網。建議抵達後再新增 eSIM。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  if (!product) {
    ...
    if (!REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-us-canada-total-product.mjs --rebuild",
      );
      return;
    }
    const oldIds = [];
  ...
  for (const sku of ["US,CA-Total3GB-3-A0", "US,CA-Total5GB-3-A0"]) {
