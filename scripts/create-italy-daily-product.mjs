/**
 * 建立「義大利 每日型 eSIM」（單一電信變體）
 *   Iliad / TIM + ← EU 32-Daily*-A0（FR IP、orange、每日高速後 128kbps、150%）
 *   支援 ChatGPT、TikTok、Gemini
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-italy-daily-product.mjs
 *   HKD_TO_TWD=4.5 node scripts/create-italy-daily-product.mjs --rebuild
 */
import fs from "fs";
...
const HANDLE = "italy-daily-esim";
const TITLE = "義大利 每日型 eSIM";
...
async function main() {
  ...
  const samples = [
    rows.find((r) => r.day === 6 && r.data === "每日 3GB"),
  ...
  const productMeta = {
    ...
    seo_title: "義大利 eSIM 每日型｜Iliad／TIM／WindTre｜Jeko eSIM",
    seo_description:
      "義大利每日型 eSIM，Iliad／TIM／WindTre 4G／5G。可選每日 500MB／1GB／2GB／3GB，高速用完後約 128kbps，隔日重置。支援 ChatGPT、TikTok、Gemini。建議抵達義大利覆蓋範圍後再啟用。",
    seo_keywords:
      "義大利eSIM,Rome eSIM,Iliad,TIM,WindTre,每日型,旅遊eSIM,Jeko eSIM",
    subtitle_by_carrier: {
      [TELECOM]: "Iliad／TIM／WindTre｜每日高速後約 128kbps｜法國 IP",
    },
    ...
    overview_notices_by_carrier: {
      [TELECOM]: {
        fup_notice:
          "義大利走 Iliad／TIM／WindTre。每日高速用完後降速約 128kbps，隔日重置。出網法國 IP（orange），支援 ChatGPT／TikTok／Gemini。地鐵、古蹟室內與鄉村收訊不保證。",
  ...
  const payloadBase = {
    title: TITLE,
    subtitle: "Iliad／TIM／WindTre｜每日型｜高速後約 128kbps",
    handle: HANDLE,
    description:
      "義大利每日型 eSIM，走 Iliad／TIM／WindTre 4G／5G。可選每日 500MB／1GB／2GB／3GB。每日高速用完後約 128kbps 可持續使用，隔日重置。支援 ChatGPT、TikTok、Gemini 與熱點。建議抵達義大利覆蓋範圍後再啟用。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  if (!product) {
    ...
    if (!REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-italy-daily-product.mjs --rebuild",
      );
      return;
    }
