/**
 * 建立「澳洲 吃到飽 不限流量 eSIM」
 *   OPTUS ← Australia-unlimited-*-A0（SG IP、e-ideas、FUP、85%）
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-australia-unlimited-product.mjs
 *   HKD_TO_TWD=4.5 node scripts/create-australia-unlimited-product.mjs --rebuild
 */
import fs from "fs";
...
const HANDLE = "australia-unlimited-esim";
const TITLE = "澳洲 吃到飽 不限流量 eSIM";
...
function collectRows(raw) {
  ...
  for (const p of raw) {
    ...
    if (!/^Australia-unlimited-\d+-A0$/i.test(name)) continue;
...
async function main() {
  ...
  console.log(`  ${TELECOM} ← Australia-unlimited-*-A0`);

  const rows = collectRows(await fetchPlans());
  if (!rows.length) throw new Error("找不到 Australia-unlimited-*-A0");
  ...
  const productMeta = {
    ...
    seo_title: "澳洲 eSIM 吃到飽不限流量｜Optus 5G｜Jeko eSIM",
    seo_description:
      "澳洲吃到飽不限流量 eSIM，Optus 4G／5G，FUP 可持續上網。新加坡 IP，支援 ChatGPT、TikTok、Gemini。1～30 天可選。",
    seo_keywords:
      "澳洲eSIM,Australia eSIM,Optus,吃到飽,不限流量,旅遊eSIM,Jeko eSIM",
    subtitle_by_carrier: {
      [TELECOM]: "Optus 4G／5G｜FUP 吃到飽｜新加坡 IP",
    },
    ...
    overview_notices_by_carrier: {
      [TELECOM]: {
        fup_notice:
          "Optus 漫遊吃到飽（FUP）。出網新加坡 IP，支援 ChatGPT／TikTok／Gemini。偏遠內陸收訊不保證。",
  ...
  const payloadBase = {
    title: TITLE,
    subtitle: "Optus 4G／5G｜吃到飽不限流量｜FUP｜85%",
    handle: HANDLE,
    description:
      "澳洲吃到飽不限流量 eSIM，走 Optus 4G／5G。FUP 可持續上網，新加坡 IP（APN e-ideas），支援 ChatGPT、TikTok、Gemini 與熱點。可選 1～10、15、20、25、30 天。建議抵達澳洲後再啟用。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  if (!product) {
    ...
    if (!REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-australia-unlimited-product.mjs --rebuild",
      );
      return;
    }
