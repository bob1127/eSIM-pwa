/**
 * 建立「澳洲 每日型 eSIM」
 *   OPTUS ← Australia-Daily*-A0/A1（SG IP、e-ideas、每日高速後 128kbps、180%）
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-australia-daily-product.mjs
 *   HKD_TO_TWD=4.5 node scripts/create-australia-daily-product.mjs --rebuild
 */
import fs from "fs";
...
const HANDLE = "australia-daily-esim";
const TITLE = "澳洲 每日型 eSIM";
const TELECOM = AU_TELECOM_OPTUS;
const PROFIT = 180;
...
function collectRows(raw) {
  ...
  for (const p of raw) {
    ...
    if (!/^Australia-Daily\d+(?:GB|MB)-\d+-A[01]$/i.test(name)) continue;
    if (/5\s*mbps/i.test(name) || /5\s*mbps/i.test(p.speed_desc || "")) continue;
    ...
    map.set(key, {
      ...
      networks: p.networks || p.operator || "AU:Optus[4G;LTE;5G]|",
...
async function ensureCategory(token) {
  ...
  console.log("🆕 已建立分類", id, "australia");
  return id;
}

async function main() {
  console.log(`💱 HKD→TWD ${HKD_TO_TWD} · ${TELECOM} ${PROFIT}%`);
  console.log(`  ${TELECOM} ← Australia-Daily*-A0/A1（128kbps）`);

  const rows = collectRows(await fetchPlans());
  if (!rows.length) throw new Error("找不到 Australia-Daily*-A0/A1");
  ...
  const productMeta = {
    ...
    seo_title: "澳洲 eSIM 每日型｜Optus 5G｜Jeko eSIM",
    seo_description:
      "澳洲每日型 eSIM，Optus 4G／5G。可選每日 500MB／1GB／2GB／3GB，高速用完後約 128kbps，隔日重置。新加坡 IP，支援 ChatGPT、TikTok、Gemini。",
    seo_keywords:
      "澳洲eSIM,Australia eSIM,Optus,每日型,旅遊eSIM,Jeko eSIM",
    subtitle_by_carrier: {
      [TELECOM]: "Optus 4G／5G｜每日高速後約 128kbps｜新加坡 IP",
    },
    ...
    overview_notices_by_carrier: {
      [TELECOM]: {
        fup_notice:
          "Optus 漫遊每日型。每日高速用完後降速約 128kbps，隔日重置。出網新加坡 IP，支援 ChatGPT／TikTok／Gemini。偏遠內陸收訊不保證。",
        activation_notice: "建議抵達澳洲後再安裝／啟用 eSIM",
      },
    },
  ...
  const payloadBase = {
    title: TITLE,
    subtitle: "Optus 4G／5G｜每日型｜高速後約 128kbps｜180%",
    handle: HANDLE,
    description:
      "澳洲每日型 eSIM，走 Optus 4G／5G。可選每日 500MB／1GB／2GB／3GB，1～10、15、20、25、30 天。每日高速用完後約 128kbps 可持續使用，隔日重置。新加坡 IP（APN e-ideas），支援 ChatGPT、TikTok、Gemini 與熱點。建議抵達澳洲後再啟用。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  if (!product) {
    ...
    if (!REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-australia-daily-product.mjs --rebuild",
      );
      return;
    }
  ...
  console.log(`前台: /product/australia/${HANDLE}/`);
