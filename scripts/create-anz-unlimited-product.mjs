/**
 * 建立「紐澳 (紐西蘭、澳洲雙切換) 吃到飽 不限流量 eSIM」
 *   VODAFONE + NZ V ← Australia&New Zealand-unlimited-*-B0
 *   （PL IP、APN plus、FUP、60%）
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-anz-unlimited-product.mjs
 *   HKD_TO_TWD=4.5 node scripts/create-anz-unlimited-product.mjs --rebuild
 */
import fs from "fs";
...
} from "../content/product-detailed/anz-key-features.js";
...
const HANDLE = "anz-unlimited-esim";
const TITLE = "紐澳 (紐西蘭、澳洲雙切換) 吃到飽 不限流量 eSIM";
const TELECOM = ANZ_TELECOM_VF;
const DATA = "吃到飽";
const PROFIT = 60;
...
function collectRows(raw) {
  ...
  for (const p of raw) {
    ...
    if (!/^Australia\s*&?\s*New Zealand-unlimited-\d+-B0$/i.test(name)) continue;
    ...
    map.set(day, {
      ...
      networks:
        p.networks ||
        p.operator ||
        "AU:Vodafone[4G;LTE;5G]|NZ:Vodafone[4G;LTE;5G],Spark[4G;LTE;5G]|",
  ...
  return {
    ...
    metadata: {
      ...
      attributes: {
        ...
        coverage: "紐澳（澳洲＋紐西蘭）",
        apps: "熱點分享,ChatGPT,TikTok,Gemini；紐澳雙國可用",
      },
    },
  };
}
...
async function ensureCategory(token) {
  ...
  const existing = (cats || []).find(
    (c) =>
      c.handle === "anz" ||
      c.handle === "au-nz" ||
      c.handle === "australia-new-zealand" ||
      /^紐澳$/.test(String(c.name || "")) ||
      /^澳紐$/.test(String(c.name || "")),
  );
  if (existing) {
  ...
  console.log("🆕 已建立分類", id, "anz");
  return id;
}

async function main() {
  console.log(`💱 HKD→TWD ${HKD_TO_TWD} · ${TELECOM} ${PROFIT}%`);
  console.log(`  ${TELECOM} ← Australia&New Zealand-unlimited-*-B0`);

  const rows = collectRows(await fetchPlans());
  if (!rows.length) {
    throw new Error("找不到 Australia&New Zealand-unlimited-*-B0");
  }

  const samples = [
    rows.find((r) => r.day === 10),
    rows.find((r) => r.day === 15),
  ].filter(Boolean);
  ...
  const productMeta = {
    ...
    seo_title: "紐澳 eSIM 吃到飽不限流量｜Vodafone／Spark｜Jeko eSIM",
    seo_description:
      "紐澳（紐西蘭、澳洲雙切換）吃到飽不限流量 eSIM。澳洲 Vodafone、紐西蘭 Vodafone／Spark 4G／5G，FUP 可持續上網。波蘭 IP，支援 ChatGPT、TikTok、Gemini。10／15 天可選。",
    seo_keywords:
      "紐澳eSIM,澳紐eSIM,澳洲紐西蘭eSIM,Vodafone,Spark,吃到飽,不限流量,旅遊eSIM,Jeko eSIM",
    subtitle_by_carrier: {
      [TELECOM]: "AU Vodafone＋NZ Vodafone／Spark｜FUP 吃到飽｜波蘭 IP",
    },
    ...
    overview_notices_by_carrier: {
      [TELECOM]: {
        fup_notice:
          "紐澳雙切換漫遊吃到飽（FUP）。澳洲 Vodafone，紐西蘭 Vodafone／Spark。出網波蘭 IP，支援 ChatGPT／TikTok／Gemini。偏遠內陸與南島山區收訊不保證。",
        activation_notice: "建議抵達澳洲或紐西蘭後再安裝／啟用 eSIM",
      },
    },
  ...
  const payloadBase = {
    title: TITLE,
    subtitle: "VODAFONE＋NZ V｜紐澳雙切換｜吃到飽不限流量｜FUP｜60%",
    handle: HANDLE,
    description:
      "紐澳（紐西蘭、澳洲雙切換）吃到飽不限流量 eSIM。澳洲走 Vodafone 4G／5G，紐西蘭走 Vodafone／Spark 4G／5G。FUP 可持續上網，波蘭 IP（APN plus），支援 ChatGPT、TikTok、Gemini 與熱點。目前可選 10、15 天。建議抵達澳洲或紐西蘭後再啟用。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  if (!product) {
    ...
    if (!REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-anz-unlimited-product.mjs --rebuild",
      );
      return;
    }
  ...
  console.log(`前台: /product/anz/${HANDLE}/`);
