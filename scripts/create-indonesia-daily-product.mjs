/**
 * 建立「印尼 eSIM 每日型」— GPT✅／TikTok✅ 線路，利潤 100%
 *   Telkomsel / XL ← Indonesia-Daily*（SG IP、e-ideas、Support Tiktok ChatGPT）
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-indonesia-daily-product.mjs --rebuild
 */
import fs from "fs";
...
const HANDLE = "indonesia-daily-esim";
...
function parseDailyCap(name) {
  const m = String(name || "").match(/Daily(\d+(?:\.\d+)?(?:GB|MB))/i);
  return m ? m[1].toUpperCase() : "";
}

function toDailyLabel(cap) {
  return cap ? `每日 ${cap}` : "";
}
...
function isGptTikTokDailyPlan(p) {
  const rawName = String(p.name || p.channel_dataplan_name || "").trim();
  if (!/^Indonesia\s*-Daily/i.test(rawName)) return false;
...
function collectRows(raw) {
  const map = new Map();
  for (const p of raw) {
    if (!isGptTikTokDailyPlan(p)) continue;
    const day = Number(p.day) || 0;
    if (!day) continue;

    const rawName = String(p.name || p.channel_dataplan_name || "").trim();
    const sku = normalizeSku(rawName);
    const dailyCap = parseDailyCap(sku);
    const data_amount = toDailyLabel(dailyCap);
  ...
  return [...map.values()].sort(
    (a, b) =>
      dataRank(a.data_amount) - dataRank(b.data_amount) || a.day - b.day,
  );
}
...
function toVariant(row) {
  const speedRule =
    row.speed_desc ||
    `${row.data_amount} 高速額度，當日用完後降速至約 128kbps（隔日重置）`;
...
async function main() {
  ...
  console.log("  篩選：Indonesia-Daily*（SG IP・GPT✅ TikTok✅）");

  const rows = collectRows(await fetchPlans());
  if (!rows.length) {
    throw new Error("找不到 Indonesia-Daily*（GPT/TikTok 線路）");
  }

  for (const r of [
    rows.find((x) => x.daily_cap === "500MB" && x.day === 1),
    rows.find((x) => x.daily_cap === "1GB" && x.day === 1),
    rows.find((x) => x.daily_cap === "3GB" && x.day === 7),
  ].filter(Boolean)) {
    console.log(
  ...
  const productMeta = {
    ...
    seo_title: "印尼 eSIM 每日型｜Telkomsel / XL｜GPT・TikTok 可用",
    seo_description:
      "印尼每日型 eSIM：每日 500MB～3GB 高速流量，用完降速 128kbps 隔日重置，支援 ChatGPT、TikTok。",
    seo_keywords:
      "印尼eSIM,峇里島eSIM,每日型,Telkomsel,XL,TikTok,ChatGPT,旅遊eSIM,Jeko eSIM",
    subtitle_by_carrier: {
      [TELECOM]: "每日型・Telkomsel／XL・GPT／TikTok 可用",
    },
    carrier_specs_by_carrier: {
      [TELECOM]: {
        ...
        speed_rule: "每日高速用完後降速至約 128kbps（隔日重置）",
    ...
    key_features_by_carrier: {
      [TELECOM]: telkomselXlDailyKeyFeatures(),
    },
    overview_notices_by_carrier: {
      [TELECOM]: {
        fup_notice:
          "每日型：當日高速額度用完後降速至約 128kbps，隔日重置。本線路支援 ChatGPT 與 TikTok（新加坡 IP 漫遊）。",
  ...
  const payloadBase = {
    title: "印尼 eSIM 每日型｜GPT・TikTok 可用",
    subtitle: "Telkomsel／XL・每日 500MB～3GB・高速用完降速 128kbps",
    handle: HANDLE,
    description:
      "印尼每日型 eSIM，Telkomsel／XL 雙網（新加坡 IP）。僅收支援 ChatGPT 與 TikTok 的線路。每日高速額度用完後降速，隔日重置。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  if (!product) {
    ...
    if (REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-indonesia-daily-product.mjs --rebuild",
      );
      return;
    }
  }
