/**
 * 建立「義大利 吃到飽 eSIM」（兩個電信變體）
 *   1) Iliad / TIM + ← EU 32-unlimited-*-A0｜限速約 10Mbps｜orange／法國 IP｜90%｜HOT SALE
 *   2) Iliad / WindTre + ← EU 36-unlimited-*-A0｜限速約 10Mbps｜cmlink／德國 IP｜90%
 *   兩線皆支援 ChatGPT、TikTok、Gemini
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-italy-unlimited-product.mjs --rebuild
 */
import fs from "fs";
...
const THUMB =
  process.env.IT_PRODUCT_THUMB ||
  `${SITE_ORIGIN}/images/${encodeURIComponent("分類eSIM-義大利.png")}`;

const HANDLE = "italy-unlimited-esim";
const TITLE = "義大利 吃到飽 eSIM";
const IT_NETWORKS_32 = "Iliad／TIM／WindTre";
const IT_NETWORKS_41 = "Iliad／WindTre";

const LINES = [
  {
    telecom: IT_TELECOM_32,
    profit: 90,
    ...
    networkLabel: "Iliad／TIM／WindTre 4G/5G",
    coverage: "義大利",
    subtitle: `${IT_NETWORKS_32}｜限速約 10Mbps｜法國 IP`,
    gpt: true,
    tiktok: true,
    gemini: true,
    apps: "熱點分享,ChatGPT,TikTok,Gemini",
    fupNotice:
      "方案限速約 10Mbps 吃到飽。義大利走 Iliad／TIM／WindTre。出網法國 IP（orange），支援 ChatGPT／TikTok／Gemini。地鐵、古蹟室內與鄉村收訊不保證。",
  },
  {
    telecom: IT_TELECOM_41,
    profit: 90,
    ...
    throttleKind: "capped",
    speedRule: "限速約 10Mbps 吃到飽",
    networkLabel: "Iliad／WindTre 4G/5G",
    coverage: "義大利",
    subtitle: `${IT_NETWORKS_41}｜限速約 10Mbps｜德國 IP`,
    gpt: true,
    tiktok: true,
    gemini: true,
    apps: "熱點分享,ChatGPT,TikTok,Gemini",
    fupNotice:
      "方案限速約 10Mbps 吃到飽。義大利走 Iliad／WindTre。出網德國 IP（cmlink），支援 ChatGPT／TikTok／Gemini。地鐵、古蹟室內與鄉村收訊不保證。",
  },
];
...
async function upsertMergedProduct(token, categoryId, raw) {
  ...
  const productMeta = {
    ...
    is_native: false,
    plan_kind: "unlimited",
    hot_sale_telecoms: [IT_TELECOM_32],
    carrier_profit_by_carrier: Object.fromEntries(
      LINES.map((s) => [s.telecom, s.profit]),
    ),
    seo_title: "義大利 eSIM 吃到飽｜Iliad／TIM／WindTre｜Jeko eSIM",
    seo_description:
      "義大利吃到飽 eSIM，義大利網路 Iliad、TIM、WindTre，限速約 10Mbps。支援 ChatGPT、TikTok、Gemini。建議抵達義大利覆蓋範圍後再啟用。",
    seo_keywords:
      "義大利eSIM,Rome eSIM,Iliad,TIM,WindTre,吃到飽,10Mbps,旅遊eSIM,Jeko eSIM",
    ...
    overview_notices_by_carrier: Object.fromEntries(
      LINES.map((s) => [
        s.telecom,
        {
          ...
          activation_notice: "建議抵達義大利覆蓋範圍後再安裝／啟用 eSIM",
        },
      ]),
    ),
  ...
  const payloadBase = {
    title: TITLE,
    subtitle: "Iliad／TIM／WindTre｜吃到飽｜約 10Mbps",
    handle: HANDLE,
    description:
      "義大利吃到飽 eSIM，義大利可走 Iliad、TIM、WindTre 4G／5G，限速約 10Mbps。支援 ChatGPT、TikTok、Gemini 與熱點。建議抵達義大利覆蓋範圍後再啟用。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  if (!product) {
    ...
    if (!REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-italy-unlimited-product.mjs --rebuild",
      );
      return product;
    }
    const oldIds = [];
  ...
  console.log(`前台: /product/italy/${HANDLE}/`);
