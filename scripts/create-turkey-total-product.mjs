/**
 * 建立「土耳其 總量型 eSIM」
 *   AVEA TURKEY / VODAFONE TURKEY + ← Turkey-Total*-A1
 *   （GB/PL IP、plus、高速後 128kbps、120%）
 *   不含 A0／B2 斷網 terminate。
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-turkey-total-product.mjs
 *   HKD_TO_TWD=4.5 node scripts/create-turkey-total-product.mjs --rebuild
 */
import fs from "fs";
...
const HANDLE = "turkey-total-esim";
const TITLE = "土耳其 總量型 eSIM";
...
function collectRows(raw) {
  ...
  for (const p of raw) {
    ...
    if (!/^Turkey-Total\d+GB-\d+-A1$/i.test(name)) continue;
...
async function main() {
  ...
  console.log(`  ${TELECOM} ← Turkey-Total*-A1（128kbps）`);

  const rows = collectRows(await fetchPlans());
  if (!rows.length) throw new Error("找不到 Turkey-Total*-A1（128kbps）");

  const samples = [
    rows.find((r) => r.day === 3 && r.data === "3GB"),
    rows.find((r) => r.day === 3 && r.data === "5GB"),
    rows.find((r) => r.day === 3 && r.data === "10GB"),
  ].filter(Boolean);
  ...
  const productMeta = {
    ...
    seo_title: "土耳其 eSIM 總量型｜Avea／Vodafone｜Jeko eSIM",
    seo_description:
      "土耳其總量型 eSIM，Avea／Vodafone Turkey 4G／5G。可選 1GB～50GB，高速用完後約 128kbps。英國／波蘭 IP，支援 ChatGPT、TikTok、Gemini。",
    seo_keywords:
      "土耳其eSIM,Turkey eSIM,Istanbul eSIM,Avea,Vodafone,總量型,旅遊eSIM,Jeko eSIM",
    subtitle_by_carrier: {
      [TELECOM]: "Avea／Vodafone 4G／5G｜總量高速後約 128kbps｜英國／波蘭 IP",
    },
    ...
    overview_notices_by_carrier: {
      [TELECOM]: {
        fup_notice:
          "Avea／Vodafone Turkey 漫遊總量型。高速用完後降速約 128kbps。出網英國／波蘭 IP，支援 ChatGPT／TikTok／Gemini。卡帕多奇亞與偏遠地區收訊不保證。",
  ...
  const payloadBase = {
    title: TITLE,
    subtitle: "AVEA／VODAFONE＋｜總量型｜高速後約 128kbps｜120%",
    handle: HANDLE,
    description:
      "土耳其總量型 eSIM，走 Avea／Vodafone Turkey 4G／5G。可選 1GB／3GB／5GB／10GB／20GB／30GB／50GB，3／5／7／10／15／20／25／30 天。高速用完後約 128kbps 可持續使用。英國／波蘭 IP（APN plus），支援 ChatGPT、TikTok、Gemini 與熱點。建議抵達土耳其後再啟用。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  if (!product) {
    ...
    if (!REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-turkey-total-product.mjs --rebuild",
      );
      return;
    }
