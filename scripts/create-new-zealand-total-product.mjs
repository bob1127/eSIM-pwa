/**
 * 建立「紐西蘭 總量型 eSIM」
 *   VODAFONE + ← New Zealand -Total*GB-*-A0（GB/PL IP、plus、高速後 128kbps、85%）
 *   不含 B0/B1 Spark 雙網、不含用完斷網 1GB/2GB
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-new-zealand-total-product.mjs
 *   HKD_TO_TWD=4.5 node scripts/create-new-zealand-total-product.mjs --rebuild
 */
import fs from "fs";
...
const HANDLE = "new-zealand-total-esim";
const TITLE = "紐西蘭 總量型 eSIM";
...
function collectRows(raw) {
  ...
  for (const p of raw) {
    ...
    if (!/^New Zealand\s*-Total\d+GB-\d+-A0$/i.test(name)) continue;
...
async function main() {
  ...
  console.log(`  ${TELECOM} ← New Zealand -Total*-A0（128kbps）`);

  const rows = collectRows(await fetchPlans());
  if (!rows.length) throw new Error("找不到 New Zealand -Total*-A0（128kbps）");
  ...
  const productMeta = {
    ...
    seo_title: "紐西蘭 eSIM 總量型｜Vodafone 5G｜Jeko eSIM",
    seo_description:
      "紐西蘭總量型 eSIM，Vodafone 4G／5G。可選 3GB～50GB，高速用完後約 128kbps。英國／波蘭 IP，支援 ChatGPT、TikTok、Gemini。",
    seo_keywords:
      "紐西蘭eSIM,New Zealand eSIM,Vodafone,總量型,旅遊eSIM,Jeko eSIM",
    subtitle_by_carrier: {
      [TELECOM]: "Vodafone 4G／5G｜總量高速後約 128kbps｜英國／波蘭 IP",
    },
    ...
    overview_notices_by_carrier: {
      [TELECOM]: {
        fup_notice:
          "Vodafone 漫遊總量型。高速用完後降速約 128kbps。出網英國／波蘭 IP，支援 ChatGPT／TikTok／Gemini。南島山區與偏遠公路收訊不保證。",
  ...
  const payloadBase = {
    title: TITLE,
    subtitle: "VODAFONE＋｜總量型｜高速後約 128kbps｜85%",
    handle: HANDLE,
    description:
      "紐西蘭總量型 eSIM，走 Vodafone 4G／5G。可選 3GB／5GB／10GB／20GB／30GB／50GB，3～30 天。高速用完後約 128kbps 可持續使用。英國／波蘭 IP（APN plus），支援 ChatGPT、TikTok、Gemini 與熱點。建議抵達紐西蘭後再啟用。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  if (!product) {
    ...
    if (!REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-new-zealand-total-product.mjs --rebuild",
      );
      return;
    }
