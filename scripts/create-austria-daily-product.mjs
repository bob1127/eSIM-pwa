/**
 * 建立「奧地利 每日型 eSIM」（單一商品、兩個電信變體）
 *   1) A1 / H3G + ｜歐41｜每日高速後 128kbps｜plus／英國波蘭 IP｜90%
 *   2) A1 / Three + ｜歐32｜每日高速後 128kbps｜orange／法國 IP｜150%
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-austria-daily-product.mjs
 *   HKD_TO_TWD=4.5 node scripts/create-austria-daily-product.mjs --rebuild
 */
import fs from "fs";
...
const HANDLE = "austria-daily-esim";
const TITLE = "奧地利 每日型 eSIM";
const DATA_ORDER = ["每日 500MB", "每日 1GB", "每日 2GB", "每日 3GB"];

const LINES = [
  {
    telecom: AT_TELECOM_41,
    profit: 90,
    skuRe: /^EU[\s-]*41-Daily(\d+)(GB|MB)-(\d+)-A0$/i,
    skuLabel: "EU-41-Daily*-A0",
    skuSuffix: "#at41",
    defaultApn: "plus",
    defaultIp: "GB,PL",
    ipType: "英國／波蘭 IP",
    throttleKind: "128kbps",
    speedRule: "每日高速用完後降速約 128kbps 持續使用，隔日重置",
    networkLabel: "A1／H3G 4G/5G",
    coverage: "奧地利為主，含歐包 41 國（德、法、義、瑞士、捷克、匈等）",
    subtitle: "A1／H3G｜每日高速後約 128kbps｜英國／波蘭 IP",
    fupNotice:
      "奧地利走 A1／H3G（Drei）。每日高速用完後降速約 128kbps，隔日重置。出網英國／波蘭 IP（plus），支援 ChatGPT／TikTok／Gemini。",
  },
  {
    telecom: AT_TELECOM_32,
    profit: 150,
    skuRe: /^EU[\s-]*32-Daily(\d+)(GB|MB)-(\d+)-A0$/i,
    skuLabel: "EU-32-Daily*-A0",
    skuSuffix: "#atd32",
    defaultApn: "orange",
    defaultIp: "FR",
    ipType: "法國 IP",
    ...
    subtitle: "A1／Three｜每日高速後約 128kbps｜法國 IP",
    fupNotice:
      "奧地利走 A1／Three。每日高速用完後降速約 128kbps，隔日重置。出網法國 IP（orange），支援 ChatGPT／TikTok／Gemini。",
  },
];
...
function collectRows(raw, spec) {
  ...
  for (const p of raw) {
    ...
    if (/5\s*mbps/i.test(name)) continue;
...
async function upsertMergedProduct(token, categoryId, raw) {
  ...
  for (const { spec, rows } of lineRows) {
    ...
    const sample =
      rows.find((r) => r.day === 7 && r.data === "每日 3GB") || rows[0];
    if (sample) {
      console.log(
  ...
  const productMeta = {
    ...
    seo_title: "奧地利 eSIM 每日型｜A1／H3G／Three｜歐包｜Jeko eSIM",
    seo_description:
      "奧地利每日型 eSIM，奧地利網路 A1、H3G、Three。可選歐包 41 國（A1／H3G）或歐包 32 國（A1／Three），每日高速後約 128kbps。支援 ChatGPT、TikTok、Gemini。",
    seo_keywords:
      "奧地利eSIM,Vienna eSIM,A1,H3G,Drei,Three,每日型,歐洲eSIM,旅遊eSIM,Jeko eSIM",
  ...
  const payloadBase = {
    title: TITLE,
    subtitle: "A1／H3G／Three｜每日型",
    handle: HANDLE,
    description:
      "奧地利每日型 eSIM，奧地利可走 A1、H3G、Three 4G／5G。兩種方案：歐包 41 國（A1／H3G、英國／波蘭 IP）與歐包 32 國（A1／Three、法國 IP）。可選每日 500MB／1GB／2GB／3GB，高速用完後約 128kbps，隔日重置。支援 ChatGPT、TikTok、Gemini 與熱點。建議抵達奧地利覆蓋範圍後再啟用。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  if (!product) {
    ...
    if (!REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-austria-daily-product.mjs --rebuild",
      );
      return product;
    }
    const oldIds = [];
