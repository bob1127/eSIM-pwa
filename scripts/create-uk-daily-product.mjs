/**
 * 建立「英國 每日型 eSIM」
 *   EE / Three + ← Europe 34 countries-Daily*-B0（PL IP、internet、每日高速後 512kbps）
 *   未滿 60 天 110%；60／90 天 85%
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-uk-daily-product.mjs
 *   HKD_TO_TWD=4.5 node scripts/create-uk-daily-product.mjs --rebuild
 */
import fs from "fs";
...
const HANDLE = "uk-daily-esim";
const TITLE = "英國 每日型 eSIM";
const TELECOM = UK_TELECOM_EE34;
const PROFIT = 110;
const PROFIT_LONG = 85; // 60天、90天
...
const COVERAGE =
  "英國為主，含歐包 34 國（法、德、義、西、瑞士、荷、比、奧等）";
...
function collectRows(raw) {
  ...
  for (const p of raw) {
    ...
    if (
      !/^Europe[\s-]+34[\s-]+countries-Daily\d+(?:GB|MB)-\d+-B0$/i.test(name)
    )
      continue;
    if (!is512kbps(p)) continue;
    ...
    map.set(`${day}|${data}`, {
      ...
      rule_desc: p.rule_desc || "unlimited 512kbps",
...
async function main() {
  console.log(`💱 HKD→TWD ${HKD_TO_TWD} · ${TELECOM} ${PROFIT}%／60天+ ${PROFIT_LONG}%`);
  console.log(`  ${TELECOM} ← Europe 34 countries-Daily*-B0（512kbps）`);

  const rows = collectRows(await fetchPlans());
  if (!rows.length) {
    throw new Error("找不到 Europe 34 countries-Daily*-B0（512kbps）");
  }

  const samples = [
    rows.find((r) => r.day === 5 && r.data === "每日 3GB"),
    rows.find((r) => r.day === 60 && r.data === "每日 1GB"),
    rows.find((r) => r.day === 90 && r.data === "每日 3GB"),
    rows.find((r) => r.day === 90),
    rows.find((r) => r.day === 60),
  ].filter(Boolean);
  const seen = new Set();
  ...
  const productMeta = {
    ...
    seo_title: "英國 eSIM 每日型｜EE／Three｜降速512kbps｜Jeko eSIM",
    seo_description:
      "英國每日型 eSIM，EE／Three／Vodafone／O2 4G／5G，並含歐洲 34 國。可選每日 500MB／1GB／2GB／3GB，高速用完後約 512kbps，隔日重置。波蘭 IP，支援 ChatGPT、TikTok、Gemini。",
    seo_keywords:
      "英國eSIM,London eSIM,EE,Three,每日型,歐洲eSIM,旅遊eSIM,Jeko eSIM",
    subtitle_by_carrier: {
      [TELECOM]: "EE／Three｜每日高速後約 512kbps｜波蘭 IP",
    },
    ...
    overview_notices_by_carrier: {
      [TELECOM]: {
        fup_notice:
          "英國走 EE／Three／Vodafone／O2。每日高速用完後降速約 512kbps，隔日重置。出網波蘭 IP，支援 ChatGPT／TikTok／Gemini。古蹟室內與鄉村收訊不保證。",
  ...
  const payloadBase = {
    title: TITLE,
    subtitle: "EE／Three＋｜每日型｜高速後約 512kbps",
    handle: HANDLE,
    description:
      "英國每日型 eSIM，走 EE／Three／Vodafone／O2 4G／5G，同一張含歐洲 34 國（法、德、義、西、瑞士、荷、比、奧等）。可選每日 500MB／1GB／2GB／3GB。每日高速用完後約 512kbps 可持續使用，隔日重置。波蘭 IP（APN internet／internetipv6），支援 ChatGPT、TikTok、Gemini 與熱點。建議抵達英國覆蓋範圍後再啟用。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  if (!product) {
    ...
    if (!REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-uk-daily-product.mjs --rebuild",
      );
      return;
    }
