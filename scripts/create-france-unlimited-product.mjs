/**
 * 建立「法國 吃到飽 eSIM」
 *   ORANGE + ← Europe 34 countries-unlimited-*-B0
 *   （PL IP、APN internet / internetipv6、FUP、85%）
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-france-unlimited-product.mjs
 *   HKD_TO_TWD=4.5 node scripts/create-france-unlimited-product.mjs --rebuild
 */
import fs from "fs";
...
const HANDLE = "france-unlimited-esim";
const TITLE = "法國 吃到飽 eSIM";
...
const COVERAGE =
  "法國為主，含歐包 34 國（英、德、義、西、瑞士、荷、比、奧等）";
...
function collectRows(raw) {
  ...
  for (const p of raw) {
    ...
    if (!/^Europe\s+34\s+countries-unlimited-\d+-B0$/i.test(name)) continue;
  ...
  return {
    ...
    metadata: {
      ...
      attributes: {
        ...
        coverage: COVERAGE,
        apps: "熱點分享,ChatGPT,TikTok,Gemini；歐包34國可用",
      },
    },
  };
}
...
async function main() {
  ...
  console.log(`  ${TELECOM} ← Europe 34 countries-unlimited-*-B0`);

  const rows = collectRows(await fetchPlans());
  if (!rows.length) {
    throw new Error("找不到 Europe 34 countries-unlimited-*-B0");
  }
  ...
  const productMeta = {
    ...
    seo_title: "法國 eSIM 吃到飽｜Orange／Bouygues／Free｜歐包34國｜Jeko eSIM",
    seo_description:
      "法國吃到飽 eSIM，Orange、Bouygues、Free Mobile 4G／5G，並含歐洲 34 國。FUP 不限流量，波蘭 IP，支援 ChatGPT、TikTok、Gemini。1～90 天可選。",
    seo_keywords:
      "法國eSIM,Paris eSIM,Orange,Bouygues,Free Mobile,吃到飽,歐洲eSIM,旅遊eSIM,Jeko eSIM",
    subtitle_by_carrier: {
      [TELECOM]: "Orange／Bouygues／Free｜歐包 34 國｜FUP 吃到飽｜波蘭 IP",
    },
  ...
  if (!product) {
    ...
    if (!REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-france-unlimited-product.mjs --rebuild",
      );
      return;
    }
