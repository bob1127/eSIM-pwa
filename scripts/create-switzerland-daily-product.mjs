/**
 * 建立「瑞士 每日型 eSIM」
 *   Swisscom / Sunrise + ← Europe 34 countries-Daily*-B0（PL IP、internet、每日高速後 512kbps、60%）
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-switzerland-daily-product.mjs
 *   HKD_TO_TWD=4.5 node scripts/create-switzerland-daily-product.mjs --rebuild
 */
import fs from "fs";
...
const HANDLE = "switzerland-daily-esim";
const TITLE = "瑞士 每日型 eSIM";
const TELECOM = CH_TELECOM_34;
const PROFIT = 60;
const PROFIT_LONG = 60;
...
async function main() {
  ...
  const productMeta = {
    ...
    seo_title: "瑞士 eSIM 每日型｜Swisscom／Sunrise／Salt｜Jeko eSIM",
    seo_description:
      "瑞士每日型 eSIM，Swisscom／Sunrise／Salt 4G／5G。可選每日 500MB／1GB／2GB／3GB，高速用完後約 512kbps，隔日重置。支援 ChatGPT、TikTok、Gemini。建議抵達瑞士覆蓋範圍後再啟用。",
    seo_keywords:
      "瑞士eSIM,Zurich eSIM,Swisscom,Sunrise,Salt,每日型,旅遊eSIM,Jeko eSIM",
    subtitle_by_carrier: {
      [TELECOM]: "Swisscom／Sunrise／Salt｜每日高速後約 512kbps｜波蘭 IP",
    },
    ...
    overview_notices_by_carrier: {
      [TELECOM]: {
        fup_notice:
          "瑞士走 Swisscom／Sunrise／Salt。每日高速用完後降速約 512kbps，隔日重置。出網波蘭 IP，支援 ChatGPT／TikTok／Gemini。阿爾卑斯山區、隧道與室內收訊不保證。",
  ...
  const payloadBase = {
    title: TITLE,
    subtitle: "Swisscom／Sunrise／Salt｜每日型｜高速後約 512kbps",
    handle: HANDLE,
    description:
      "瑞士每日型 eSIM，走 Swisscom／Sunrise／Salt 4G／5G。可選每日 500MB／1GB／2GB／3GB。每日高速用完後約 512kbps 可持續使用，隔日重置。支援 ChatGPT、TikTok、Gemini 與熱點。建議抵達瑞士覆蓋範圍後再啟用。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  if (!product) {
    ...
    if (!REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-switzerland-daily-product.mjs --rebuild",
      );
      return;
    }
