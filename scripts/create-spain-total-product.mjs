/**
 * 建立「西班牙 總量型 eSIM」（單一電信變體）
 *   Orange / Movistar + ← EU 32-Total*-A0（FR IP、orange、高速後 128kbps、90%）
 *   支援 ChatGPT、TikTok、Gemini
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-spain-total-product.mjs --rebuild
 */
import fs from "fs";
...
const HANDLE = "spain-total-esim";
const TITLE = "西班牙 總量型 eSIM";
const TELECOM = ES_TELECOM_32_OM;
const PROFIT = 90;
...
const NETWORK_LABEL = "Orange／Movistar 4G/5G";
const COVERAGE = "西班牙";
...
async function main() {
  ...
  const productMeta = {
    ...
    seo_title: "西班牙 eSIM 總量型｜Orange／Movistar｜Jeko eSIM",
    seo_description:
      "西班牙總量型 eSIM，Orange／Movistar 4G／5G。可選 1GB～50GB，高速用完後降速約 128kbps。支援 ChatGPT、TikTok、Gemini。建議抵達西班牙覆蓋範圍後再啟用。",
    seo_keywords:
      "西班牙eSIM,Madrid eSIM,Orange,Movistar,總量型,旅遊eSIM,Jeko eSIM",
    subtitle_by_carrier: {
      [TELECOM]: "Orange／Movistar｜總量高速後約 128kbps｜法國 IP",
    },
    ...
    overview_notices_by_carrier: {
      [TELECOM]: {
        fup_notice:
          "西班牙走 Orange／Movistar。高速額度用完後降速約 128kbps 可持續使用。出網法國 IP（orange），支援 ChatGPT／TikTok／Gemini。地鐵、古蹟室內與鄉村收訊不保證。",
  ...
  const payloadBase = {
    title: TITLE,
    subtitle: "Orange／Movistar｜總量型｜高速後約 128kbps",
    handle: HANDLE,
    description:
      "西班牙總量型 eSIM，走 Orange／Movistar 4G／5G。可選 1GB／2GB／3GB／5GB／10GB／20GB／30GB／50GB。高速額度用完後降速約 128kbps 可持續使用。支援 ChatGPT、TikTok、Gemini 與熱點。建議抵達西班牙覆蓋範圍後再啟用。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  if (!product) {
    ...
    if (!REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-spain-total-product.mjs --rebuild",
      );
      return;
    }
