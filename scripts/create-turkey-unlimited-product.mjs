/**
 * 建立「土耳其 吃到飽 不限流量 eSIM」
 *   AVEA TURKEY / VODAFONE TURKEY + ← Turkey-unlimited-*-A1
 *   （每日 1GB 高速後限速約 10Mbps、APN plus、GB/PL IP、120%）
 *   不含 A0 真 FUP、不含 B2。
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-turkey-unlimited-product.mjs
 *   HKD_TO_TWD=4.5 node scripts/create-turkey-unlimited-product.mjs --rebuild
 */
import fs from "fs";
...
const HANDLE = "turkey-unlimited-esim";
const TITLE = "土耳其 吃到飽 不限流量 eSIM";
...
function collectRows(raw) {
  ...
  for (const p of raw) {
    ...
    if (!/^Turkey-unlimited-\d+-A1$/i.test(name)) continue;
  ...
  return {
    ...
    metadata: {
      ...
      attributes: {
        ...
        data_amount: r.data,
        telecom: r.telecom,
        network: "Avea／Vodafone Turkey 4G/5G",
        ip_type: "英國／波蘭 IP",
        route_type: "漫遊",
        hotspot: true,
        gpt: true,
...
async function main() {
  ...
  console.log(`  ${TELECOM} ← Turkey-unlimited-*-A1（10Mbps）`);

  const rows = collectRows(await fetchPlans());
  if (!rows.length) {
    throw new Error("找不到 Turkey-unlimited-*-A1（10Mbps）");
  }
  ...
  const productMeta = {
    ...
    seo_title: "土耳其 eSIM 吃到飽｜Avea／Vodafone｜每日1GB後10Mbps｜Jeko eSIM",
    seo_description:
      "土耳其吃到飽 eSIM，Avea／Vodafone Turkey 4G／5G。每日 1GB 高速後限速約 10Mbps 可持續上網。英國／波蘭 IP，支援 ChatGPT、TikTok、Gemini。1～30 天可選。",
    seo_keywords:
      "土耳其eSIM,Turkey eSIM,Istanbul eSIM,Avea,Vodafone,吃到飽,10Mbps,旅遊eSIM,Jeko eSIM",
    subtitle_by_carrier: {
      [TELECOM]: "Avea／Vodafone 4G／5G｜每日 1GB 高速後約 10Mbps｜英國／波蘭 IP",
    },
    ...
    overview_notices_by_carrier: {
      [TELECOM]: {
        fup_notice:
          "Avea／Vodafone Turkey 漫遊吃到飽。每日 1GB 高速用完後限速約 10Mbps，隔日重置。出網英國／波蘭 IP，支援 ChatGPT／TikTok／Gemini。卡帕多奇亞與偏遠地區收訊不保證。",
  ...
  const payloadBase = {
    title: TITLE,
    subtitle: "AVEA／VODAFONE＋｜每日 1GB 高速後約 10Mbps｜120%",
    handle: HANDLE,
    description:
      "土耳其吃到飽不限流量 eSIM，走 Avea／Vodafone Turkey 4G／5G。每日 1GB 高速用完後限速約 10Mbps 可持續使用，隔日重置。英國／波蘭 IP（APN plus），支援 ChatGPT、TikTok、Gemini 與熱點。可選 1～10、15、20、25、30 天。建議抵達土耳其後再啟用。不含 A0 真 FUP 方案。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  if (!product) {
    ...
    if (!REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-turkey-unlimited-product.mjs --rebuild",
      );
      return;
    }
