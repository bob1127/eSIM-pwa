/**
 * 建立「奧地利 吃到飽 eSIM」（單一商品、兩個電信變體）
 *   1) Drei / A1 + ｜歐36｜限速約 10Mbps｜cmlink／德國 IP｜65%
 *   2) A1 / Three + ｜歐32｜限速約 10Mbps｜orange／法國 IP｜110%
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-austria-unlimited-product.mjs
 *   HKD_TO_TWD=4.5 node scripts/create-austria-unlimited-product.mjs --rebuild
 */
import fs from "fs";
...
const HANDLE = "austria-unlimited-esim";
const TITLE = "奧地利 吃到飽 eSIM";
const AT_NETWORKS_36 = "Drei／A1";
const AT_NETWORKS_32 = "A1／Three";

const LINES = [
  {
    telecom: AT_TELECOM_36,
    profit: 65,
    ...
    networkLabel: "Drei／A1 4G/5G",
    coverage: "奧地利為主，含歐包 36 國（德、法、義、瑞士、捷克、匈等）",
    subtitle: `${AT_NETWORKS_36}｜限速約 10Mbps｜德國 IP`,
    fupNotice:
      "方案限速約 10Mbps 吃到飽。奧地利走 Drei（Hutchison）／A1。出網德國 IP（cmlink），支援 ChatGPT／TikTok／Gemini。",
  },
  {
    ...
    subtitle: `${AT_NETWORKS_32}｜限速約 10Mbps｜法國 IP`,
    fupNotice:
      "方案限速約 10Mbps 吃到飽。奧地利走 A1／Three。出網法國 IP（orange），支援 ChatGPT／TikTok／Gemini。",
  },
];
...
async function upsertMergedProduct(token, categoryId, raw) {
  ...
  const productMeta = {
    ...
    seo_title: "奧地利 eSIM 吃到飽｜A1／Drei／Three｜歐包｜Jeko eSIM",
    seo_description:
      "奧地利吃到飽 eSIM，奧地利網路 A1、Drei、Three。可選歐包 36 國（Drei／A1、德國 IP）或歐包 32 國（A1／Three、法國 IP），限速約 10Mbps。支援 ChatGPT、TikTok、Gemini。",
    seo_keywords:
      "奧地利eSIM,Vienna eSIM,A1,Drei,Three,吃到飽,10Mbps,歐洲eSIM,旅遊eSIM,Jeko eSIM",
  ...
  const payloadBase = {
    title: TITLE,
    subtitle: "A1／Drei／Three｜吃到飽",
    handle: HANDLE,
    description:
      "奧地利吃到飽 eSIM，奧地利可走 A1、Drei、Three 4G／5G。兩種方案：歐包 36 國（Drei／A1、德國 IP）與歐包 32 國（A1／Three、法國 IP），皆為限速約 10Mbps 吃到飽。支援 ChatGPT、TikTok、Gemini 與熱點。建議抵達奧地利覆蓋範圍後再啟用。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  if (!product) {
    ...
    if (!REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-austria-unlimited-product.mjs --rebuild",
      );
      return product;
    }
    const oldIds = [];
