/**
 * 建立「土耳其 每日型 eSIM」
 *   AVEA TURKEY / VODAFONE TURKEY + ← Turkey-Daily*-A1/A3
 *   （GB/PL IP、plus、每日高速後 128kbps、150%）
 *   A3 優先、其次 A1／A2／A0；不含 5Mbps、不含 200MB、不含 B0/B2。
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-turkey-daily-product.mjs
 *   HKD_TO_TWD=4.5 node scripts/create-turkey-daily-product.mjs --rebuild
 */
import fs from "fs";
...
const HANDLE = "turkey-daily-esim";
const TITLE = "土耳其 每日型 eSIM";
const TELECOM = TR_TELECOM_AVEA;
const PROFIT = 150;
...
function suffixRank(name) {
  ...
  if (n === "A2") return 2;
  if (n === "A0") return 3;
  return 10 + parseInt(n.slice(1), 10);
}

function preferSku(prev, name, hkd) {
  if (!prev) return true;
  const pr = suffixRank(prev.sku);
  const nr = suffixRank(name);
  if (nr < pr) return true;
  if (nr === pr && hkd < prev.price_hkd) return true;
  return false;
}
...
function collectRows(raw) {
  ...
  for (const p of raw) {
    ...
    if (!/^Turkey-Daily\d+(?:GB|MB)-\d+-A\d+$/i.test(name)) continue;
    if (
    ...
    map.set(`${day}|${data}`, {
      ...
      networks:
        p.networks ||
        p.operator ||
        "TR:AVEA Turkey[4G;LTE;5G],Vodafone Turkey[4G;LTE;5G]|",
...
async function ensureCategory(token) {
  ...
  console.log("🆕 已建立分類", id, "turkey");
  return id;
}

async function main() {
  console.log(`💱 HKD→TWD ${HKD_TO_TWD} · ${TELECOM} ${PROFIT}%`);
  console.log(`  ${TELECOM} ← Turkey-Daily*-A1/A3（128kbps）`);

  const rows = collectRows(await fetchPlans());
  if (!rows.length) throw new Error("找不到 Turkey-Daily*-A1/A3（128kbps）");
  ...
  const productMeta = {
    ...
    seo_title: "土耳其 eSIM 每日型｜Avea／Vodafone｜Jeko eSIM",
    seo_description:
      "土耳其每日型 eSIM，Avea／Vodafone Turkey 4G／5G。可選每日 500MB／1GB／2GB／3GB，高速用完後約 128kbps，隔日重置。英國／波蘭 IP，支援 ChatGPT、TikTok、Gemini。",
    seo_keywords:
      "土耳其eSIM,Turkey eSIM,Istanbul eSIM,Avea,Vodafone,每日型,旅遊eSIM,Jeko eSIM",
    subtitle_by_carrier: {
      [TELECOM]: "Avea／Vodafone 4G／5G｜每日高速後約 128kbps｜英國／波蘭 IP",
    },
    ...
    overview_notices_by_carrier: {
      [TELECOM]: {
        fup_notice:
          "Avea／Vodafone Turkey 漫遊每日型。每日高速用完後降速約 128kbps，隔日重置。出網英國／波蘭 IP，支援 ChatGPT／TikTok／Gemini。卡帕多奇亞與偏遠地區收訊不保證。",
        activation_notice: "建議抵達土耳其後再安裝／啟用 eSIM",
      },
    },
  ...
  const payloadBase = {
    title: TITLE,
    subtitle: "AVEA／VODAFONE＋｜每日型｜高速後約 128kbps｜150%",
    handle: HANDLE,
    description:
      "土耳其每日型 eSIM，走 Avea／Vodafone Turkey 4G／5G。可選每日 500MB／1GB／2GB／3GB，1～10、15、20、25、30 天。每日高速用完後約 128kbps 可持續使用，隔日重置。英國／波蘭 IP（APN plus），支援 ChatGPT、TikTok、Gemini 與熱點。建議抵達土耳其後再啟用。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  if (!product) {
    ...
    if (!REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-turkey-daily-product.mjs --rebuild",
      );
      return;
    }
  ...
  console.log(`前台: /product/turkey/${HANDLE}/`);
