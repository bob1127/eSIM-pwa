/**
 * 建立／更新「馬來西亞 eSIM 每日型」— 兩個電信：
 *   1) UMobile 5G 當地 ← Malaysia-Daily*（原生 MY IP，APN my3g）— 利潤 120%、HOT SALE
 *   2) Maxis / Celcom / Digi ← Malaysia(T+C)-Daily*（漫遊 SG IP，APN e-ideas）— 利潤 95%
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-malaysia-daily-product.mjs --rebuild
 */
import fs from "fs";
...
const HANDLE = "malaysia-daily-esim";
const TELECOM_UMOBILE = "UMobile 5G 當地";
const TELECOM_DUAL = "Maxis / Celcom / Digi";
const PROFIT_BY_KIND = { umobile: 120, dual: 95 };
...
const CATEGORY_IDS = ["pcat_01KPJQNCX6FFADJ4GHYKMCS8CT"]; // malaysia
const THUMB =
...
function loadPlans(hkdToTwd) {
  const file = path.join(__dirname, "data", "malaysia-daily-plans.json");
...
async function main() {
  ...
  if (!rows.length) throw new Error("malaysia-daily-plans.json 無資料");
  ...
  for (const telecom of telecomValues) {
    const sample = rows.find((r) => r.telecom === telecom && r.day === 7);
    if (sample) {
      console.log(
        `核對 ${telecom} 7天 ${sample.data_amount}: HKD ${sample.price_hkd} → cost NT$${sample.cost_twd} → 售價 NT$${sample.retail_twd}（${sample.profit_percent}%） (${sample.sku})`,
      );
    }
  }
  ...
  const productMeta = {
    ...
    seo_title:
      "馬來西亞 eSIM 每日型｜UMobile・Maxis / Celcom / Digi｜Jeko eSIM",
    seo_description:
      "馬來西亞每日型 eSIM：UMobile 原生馬來西亞 IP（熱銷），或 Maxis / Celcom / Digi 三網漫遊。依天數與每日流量選購，支援熱點／ChatGPT／TikTok。",
    seo_keywords:
      "馬來西亞eSIM,每日型,UMobile,Maxis,Celcom,Digi,原生eSIM,旅遊eSIM,Jeko eSIM",
    subtitle_by_carrier: {
      [TELECOM_UMOBILE]:
        "每日型・UMobile 5G 當地原生・馬來西亞 IP・高速用完後降速約 128kbps",
      [TELECOM_DUAL]:
        "每日型・Maxis / Celcom / Digi・高速用完後降速約 128kbps",
    },
    ...
    key_features_by_carrier: {
      [TELECOM_UMOBILE]: umobileKeyFeatures("daily"),
      [TELECOM_DUAL]: maxisCelcomDigiKeyFeatures("daily"),
    },
    overview_notices_by_carrier: {
      [TELECOM_UMOBILE]: {
        fup_notice:
          "依所選方案提供每日高速流量（500MB／1GB／2GB／3GB）。用完後降速至約 128 kbps 可持續使用（每日重置）。UMobile 5G 當地原生網路，馬來西亞 IP。支援熱點、TikTok 與 ChatGPT。",
        activation_notice: "建議抵達馬來西亞後再安裝／啟用 eSIM",
      },
      [TELECOM_DUAL]: {
        fup_notice:
          "依所選方案提供每日高速流量（500MB／1GB／2GB／3GB）。用完後降速至約 128 kbps 可持續使用（每日重置）。Maxis／Celcom／Digi 三網自動切換。支援熱點、TikTok 與 ChatGPT。",
        activation_notice: "建議抵達馬來西亞後再安裝／啟用 eSIM",
      },
    },
  };
  ...
  const payloadBase = {
    title: "馬來西亞 eSIM 每日型  UMobile | Maxis / Celcom / Digi",
    subtitle:
      "UMobile 5G 當地（熱銷）・Maxis / Celcom / Digi 三網・每日流量型",
    handle: HANDLE,
    description:
      "馬來西亞 eSIM 每日型，兩種電信：UMobile 5G 當地原生（馬來西亞 IP，熱銷推薦，利潤結構較高），以及 Maxis / Celcom / Digi 三網漫遊（新加坡 IP）。依天數與每日高速流量選購，用完後降速約 128kbps 可持續使用，支援熱點與 ChatGPT／TikTok／Gemini。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  console.log(
    `📦 方案 ${rows.length} 筆（UMobile ${nU} @120% HOT + 三網 ${nD} @95%）`,
  );
  ...
  if (!product) {
    ...
    if (REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-malaysia-daily-product.mjs --rebuild",
      );
      return;
    }
  }
  ...
  const sU = rows.find(
    (r) => r.telecom === TELECOM_UMOBILE && r.day === 7 && /1GB/.test(r.data_amount),
  );
  const sD = rows.find(
    (r) => r.telecom === TELECOM_DUAL && r.day === 7 && /1GB/.test(r.data_amount),
  );
  if (sU) {
    console.log(
      `範例 UMobile 7天1GB: HKD ${sU.price_hkd} → cost NT$${sU.cost_twd} → 售價 NT$${sU.retail_twd}（120% HOT）`,
    );
  }
  if (sD) {
    console.log(
      `範例 三網 7天1GB: HKD ${sD.price_hkd} → cost NT$${sD.cost_twd} → 售價 NT$${sD.retail_twd}（95%）`,
    );
  }
}
