/**
 * 建立「北美 (美國、加拿大、墨西哥) 每日型 eSIM｜美國 IP」
 * SKU：US,CA,MX-Daily*-A0/A1（僅 ip=US）
 * 利潤：60%
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-north-america-daily-usip-product.mjs
 *   HKD_TO_TWD=4.5 node scripts/create-north-america-daily-usip-product.mjs --rebuild
 */
import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";
import { northAmericaDailyUsipKeyFeaturesByCarrier } from "../content/product-detailed/usa-region-key-features.js";
...
const HANDLE = "north-america-daily-usip-esim";
const TITLE = "北美 (美國、加拿大、墨西哥) 每日型 eSIM｜美國 IP";
const LINE = "漫遊線路";
const PROFIT = 60;
...
function loadPlans() {
  const file = path.join(
    __dirname,
    "data",
    "north-america-daily-usip-plans.json",
  );
...
async function main() {
  ...
  if (!rows.length) {
    throw new Error("north-america-daily-usip-plans.json 無資料");
  }

  console.log(`💱 HKD→TWD ${HKD_TO_TWD}｜利潤 ${PROFIT}%｜方案 ${rows.length}`);
  for (const s of rows.filter(
    (r) => r.day === 1 && /Daily(500MB|1GB)-1-A0$/i.test(r.sku),
  )) {
    console.log(
  ...
  const productMeta = {
    ...
    seo_title: "北美每日型 eSIM｜美國加拿大墨西哥｜美國 IP｜Jeko eSIM",
    seo_description:
      "北美（美國、加拿大、墨西哥）每日型 eSIM，美國 IP，Rogers／Movistar／Verizon／AT&T，支援熱點。",
    seo_keywords: "北美eSIM,美加墨eSIM,每日型,美國IP,Jeko eSIM",
    subtitle_by_carrier: Object.fromEntries(
      telecomValues.map((t) => [t, `美國 IP｜${t}｜每日型｜漫遊`]),
    ),
    ...
    key_features_by_carrier: northAmericaDailyUsipKeyFeaturesByCarrier(),
  };

  const payloadBase = {
    title: TITLE,
    subtitle: "美加墨｜每日型｜美國 IP｜支援熱點",
    handle: HANDLE,
    description:
      "北美（美國、加拿大、墨西哥）每日型 eSIM｜美國 IP。電信含 Rogers Canada、Movistar Mexico、Verizon USA／AT&T USA。每日流量額度用完後降速 128kbps，支援熱點。線路為漫遊（bicsapn）。建議抵達後再新增 eSIM。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  for (const sku of [
    "US,CA,MX-Daily500MB-1-A0",
    "US,CA,MX-Daily1GB-1-A0",
  ]) {
