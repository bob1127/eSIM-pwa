 ...
 *   1) 短天數｜中國電信／CSL／澳門電信
 *      CNHKMO-unlimited-*-A0｜HK IP｜ctexcel｜利潤 90%
 *   2) 長天數｜中國電信／聯通／CSL／澳門電訊
 *      CN,HK,MO(T+C)-unlimited-*-A0｜SG IP｜FUP 10Mbps｜利潤 65%
 *
...
function loadPlans(hkdToTwd) {
  ...
  const rows = [
    ...
    ...expandTelecomPlans(raw.long_tc, TELECOM_LONG, hkdToTwd),
  ];
  return rows.sort(
    (a, b) =>
...
async function main() {
  ...
  const payloadBase = {
    ...
    handle: HANDLE,
    description:
      "中國大陸、香港、澳門一卡吃到飽不限流量。提供兩個電信變體：短天數｜中國電信／CSL／澳門電信（香港 IP，利潤 90%；免 VPN 用 LINE／IG／FB）；長天數｜中國電信／聯通／CSL／澳門電訊（新加坡 IP、約 10Mbps，利潤 65%；支援 ChatGPT／TikTok／Gemini）。兩線路出網皆非中國大陸 IP，一般可免 VPN 使用 LINE、Instagram、Facebook（實際依當下路由）。支援熱點，依天數選購，抵達後安裝即可使用。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  console.log(
    `📦 方案 ${rows.length} 筆（短 ${nShort} + 長 ${nLong}）・利潤 90%／65%`,
  );
