/**
 * 建立「法國 總量型 eSIM」
 *   ORANGE + ← EU 49-Total*-A0（GB/PL IP、plus、高速後 128kbps、100%）
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-france-total-product.mjs
 *   HKD_TO_TWD=4.5 node scripts/create-france-total-product.mjs --rebuild
 */
import fs from "fs";
...
} from "../content/product-detailed/france-key-features.js";
...
const HANDLE = "france-total-esim";
const TITLE = "法國 總量型 eSIM";
const TELECOM = FR_TELECOM_ORANGE;
const PROFIT = 100;
...
const COVERAGE =
  "法國為主，含歐包 49 國（英、德、義、西、瑞士、荷、比、奧、阿爾巴尼亞等）";
...
function collectRows(raw) {
  ...
  for (const p of raw) {
    ...
    if (!/^EU\s*49-Total\d+GB-\d+-A0$/i.test(name)) continue;
  ...
  return {
    ...
    metadata: {
      ...
      attributes: {
        ...
        coverage: COVERAGE,
        apps: "熱點分享,ChatGPT,TikTok,Gemini；歐包49國可用",
      },
    },
  };
}
...
async function ensureCategory(token) {
  ...
  console.log("🆕 已建立分類", id, "france");
  return id;
}

async function main() {
  console.log(`💱 HKD→TWD ${HKD_TO_TWD} · ${TELECOM} ${PROFIT}%`);
  console.log(`  ${TELECOM} ← EU 49-Total*-A0（128kbps）`);

  const rows = collectRows(await fetchPlans());
  if (!rows.length) throw new Error("找不到 EU 49-Total*-A0（128kbps）");
  ...
  const productMeta = {
    ...
    seo_title: "法國 eSIM 總量型｜Orange／Bouygues｜歐包49國｜Jeko eSIM",
    seo_description:
      "法國總量型 eSIM，Orange／Bouygues 4G／5G，並含歐洲 49 國。可選 1GB～50GB，高速用完後約 128kbps。英國／波蘭 IP，支援 ChatGPT、TikTok、Gemini。",
    seo_keywords:
      "法國eSIM,Paris eSIM,Orange,Bouygues,總量型,歐洲eSIM,旅遊eSIM,Jeko eSIM",
    subtitle_by_carrier: {
      [TELECOM]: "Orange／Bouygues｜歐包 49 國｜總量高速後約 128kbps｜英國／波蘭 IP",
    },
    ...
    overview_notices_by_carrier: {
      [TELECOM]: {
        fup_notice:
          "歐包 49 國漫遊總量型。法國走 Orange／Bouygues。高速用完後降速約 128kbps。出網英國／波蘭 IP，支援 ChatGPT／TikTok／Gemini。古蹟室內與鄉村收訊不保證。",
        activation_notice: "建議抵達法國或歐包覆蓋範圍後再安裝／啟用 eSIM",
      },
    },
    key_features_by_carrier: frTotalKeyFeaturesByCarrier(),
  };

  const payloadBase = {
    title: TITLE,
    subtitle: "ORANGE＋｜歐包 49 國｜總量型｜高速後約 128kbps｜100%",
    handle: HANDLE,
    description:
      "法國總量型 eSIM，走 Orange／Bouygues 4G／5G，同一張含歐洲 49 國（英、德、義、西、瑞士、荷、比、奧、阿爾巴尼亞等）。可選 1GB／3GB／5GB／10GB／20GB／30GB／50GB，3／5／7／10／15／30 天。高速用完後約 128kbps 可持續使用。英國／波蘭 IP（APN plus），支援 ChatGPT、TikTok、Gemini 與熱點。建議抵達法國或歐包覆蓋範圍後再啟用。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  if (!product) {
    ...
    if (!REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-france-total-product.mjs --rebuild",
      );
      return;
    }
  ...
  console.log(`前台: /product/france/${HANDLE}/`);
