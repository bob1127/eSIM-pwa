/**
 * 建立「法國 每日型 eSIM」
 *   ORANGE + ← EU 49-Daily*-A0（GB/PL IP、plus、每日高速後 128kbps、90%）
 *   不含 5Mbps。
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-france-daily-product.mjs
 *   HKD_TO_TWD=4.5 node scripts/create-france-daily-product.mjs --rebuild
 */
import fs from "fs";
...
const HANDLE = "france-daily-esim";
const TITLE = "法國 每日型 eSIM";
...
function collectRows(raw) {
  ...
  for (const p of raw) {
    ...
    if (!/^EU\s*49[\s-]*Daily\d+(?:GB|MB)-\d+-A0$/i.test(name)) continue;
    if (
...
async function main() {
  ...
  console.log(`  ${TELECOM} ← EU 49-Daily*-A0（128kbps）`);

  const rows = collectRows(await fetchPlans());
  if (!rows.length) throw new Error("找不到 EU 49-Daily*-A0（128kbps）");
  ...
  const productMeta = {
    ...
    seo_title: "法國 eSIM 每日型｜Orange／Bouygues｜歐包49國｜Jeko eSIM",
    seo_description:
      "法國每日型 eSIM，Orange／Bouygues 4G／5G，並含歐洲 49 國。可選每日 500MB／1GB／2GB／3GB，高速用完後約 128kbps，隔日重置。英國／波蘭 IP，支援 ChatGPT、TikTok、Gemini。",
    seo_keywords:
      "法國eSIM,Paris eSIM,Orange,Bouygues,每日型,歐洲eSIM,旅遊eSIM,Jeko eSIM",
    subtitle_by_carrier: {
      [TELECOM]: "Orange／Bouygues｜歐包 49 國｜每日高速後約 128kbps｜英國／波蘭 IP",
    },
  ...
  if (!product) {
    ...
    if (!REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-france-daily-product.mjs --rebuild",
      );
      return;
    }
