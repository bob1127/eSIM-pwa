/**
 * 建立「英國 總量型 eSIM」
 *   EE / Three + ← Europe 34 countries-Total*-B0（PL IP、internet、用完斷網、110%）
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-uk-total-product.mjs
 *   HKD_TO_TWD=4.5 node scripts/create-uk-total-product.mjs --rebuild
 */
import fs from "fs";
...
const HANDLE = "uk-total-esim";
const TITLE = "英國 總量型 eSIM";
...
async function main() {
  ...
  console.log(`  ${TELECOM} ← Europe 34 countries-Total*-B0（110%）`);
  ...
  const productMeta = {
    ...
    seo_title: "英國 eSIM 總量型｜EE／Three｜Jeko eSIM",
    seo_description:
      "英國總量型 eSIM，EE／Three／Vodafone／O2 4G／5G，並含歐洲 34 國。可選 1GB～50GB，高速用完後斷網。波蘭 IP，支援 ChatGPT、TikTok、Gemini。",
    seo_keywords:
      "英國eSIM,London eSIM,EE,Three,總量型,歐洲eSIM,旅遊eSIM,Jeko eSIM",
    subtitle_by_carrier: {
      [TELECOM]: "EE／Three｜總量型用完斷網｜波蘭 IP",
    },
    ...
    overview_notices_by_carrier: {
      [TELECOM]: {
        fup_notice:
          "英國走 EE／Three／Vodafone／O2。高速額度用完後斷網。出網波蘭 IP，支援 ChatGPT／TikTok／Gemini。古蹟室內與鄉村收訊不保證。",
  ...
  const payloadBase = {
    title: TITLE,
    subtitle: "EE／Three＋｜總量型｜用完斷網",
    handle: HANDLE,
    description:
      "英國總量型 eSIM，走 EE／Three／Vodafone／O2 4G／5G，同一張含歐洲 34 國（法、德、義、西、瑞士、荷、比、奧等）。可選 1GB／3GB／5GB／10GB／20GB／30GB／50GB，3／5／7／10／15／30 天。高速額度用完後斷網。波蘭 IP（APN internet／internetipv6），支援 ChatGPT、TikTok、Gemini 與熱點。建議抵達英國覆蓋範圍後再啟用。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  if (!product) {
    ...
    if (!REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-uk-total-product.mjs --rebuild",
      );
      return;
    }
