/**
 * 建立「美國本土 每日型 eSIM｜美國 IP」
 * SKU：United States of America-Daily*-A0（及同網 US IP 每日型）
 * 利潤：95%
 * 線路：漫遊（bicsapn）｜IP：US
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-usa-mainland-daily-usip-product.mjs
 *   HKD_TO_TWD=4.5 node scripts/create-usa-mainland-daily-usip-product.mjs --rebuild
 */
import fs from "fs";
...
const TITLE = "美國本土 每日型 eSIM｜美國 IP";
const TELECOM = "Verizon USA / AT&T USA";
const LINE = "漫遊線路";
const PROFIT = 95;
...
function loadPlans() {
  ...
  return (raw.plans || [])
    .map((p) => {
      ...
      const data = p.data || "每日型";
      return {
        ...p,
        day,
        daysLabel: `${day}天`,
        telecom: p.telecom || TELECOM,
        data,
        price_hkd,
        cost_twd,
...
function toVariant(row) {
  const telecom = row.telecom || TELECOM;
  const data = row.data;
  return {
    ...
    metadata: {
      ...
      attributes: {
        ...
        telecom,
        line: LINE,
        network: "Verizon USA / AT&T USA · 4G·5G",
        ip_type: "美國 IP",
        route_type: LINE,
        hotspot: true,
        gpt: true,
...
async function main() {
  ...
  if (!rows.length) {
    throw new Error("usa-mainland-daily-usip-plans.json 無資料");
  }

  console.log(`💱 HKD→TWD ${HKD_TO_TWD}｜利潤 ${PROFIT}%｜方案 ${rows.length}`);
  for (const s of rows.filter((r) => r.day === 1)) {
    console.log(
      `   ${s.day}天 ${s.data} HKD ${s.price_hkd} → cost NT$${s.cost_twd} → 售價 NT$${s.retail_twd}`,
    );
  }
  ...
  const productMeta = {
    ...
    seo_title: "美國本土 每日型 eSIM｜美國 IP｜Jeko eSIM",
    seo_description:
      "美國本土每日型 eSIM：Verizon USA／AT&T USA、美國 IP，每日流量額度，支援熱點。",
    seo_keywords: "美國eSIM,每日型eSIM,美國IP,Verizon,AT&T,Jeko eSIM",
    subtitle_by_carrier: {
      [TELECOM]: "美國 IP｜Verizon USA / AT&T USA｜每日型｜支援熱點",
    },
  ...
  const payloadBase = {
    title: TITLE,
    subtitle: "Verizon USA / AT&T USA｜美國 IP｜每日型｜支援熱點",
    handle: HANDLE,
    description:
      "美國本土每日型 eSIM｜美國 IP。電信 Verizon USA／AT&T USA（5G），每日流量額度用完後降速 128kbps，支援熱點。線路為漫遊（bicsapn），阿拉斯加／夏威夷不保證。建議抵達後再新增 eSIM。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  if (!product) {
    ...
    if (!REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-usa-mainland-daily-usip-product.mjs --rebuild",
      );
      return;
    }
  ...
  const sample = vs.find((v) => /Daily500MB-1-A0/i.test(v.sku || ""));
  if (sample) {
    console.log(
      `核對 1天500MB: ${sample.sku} TWD ${sample.prices?.[0]?.amount ?? "?"}`,
    );
  }
}
