/**
 * 建立／更新「新加坡 eSIM 每日型」— 單一電信：
 *   M1 / Starhub ← Singapore-Daily*（smartone／HK IP）— 利潤 120%
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-singapore-daily-product.mjs --rebuild
 */
import fs from "fs";
...
const HANDLE = "singapore-daily-esim";
...
const THUMB =
  ...
  "https://www.jeko-esim.com.tw/images/新加坡esim.png";
...
function loadPlans(hkdToTwd) {
  const file = path.join(__dirname, "data", "singapore-daily-plans.json");
...
async function main() {
  ...
  if (!rows.length) throw new Error("singapore-daily-plans.json 無資料");
  ...
  const sample = rows.find(
    (r) => r.day === 1 && r.data_amount === "每日 500MB",
  );
  if (sample) {
    console.log(
      `核對 ${TELECOM} 1天 500MB: HKD ${sample.price_hkd} → cost NT$${sample.cost_twd} → 售價 NT$${sample.retail_twd}（${sample.profit_percent}%） (${sample.sku})`,
    );
  }
  ...
  const productMeta = {
    ...
    seo_title: "新加坡 eSIM 每日型｜M1 / Starhub｜Jeko eSIM",
    seo_description:
      "新加坡每日型 eSIM：M1／Starhub 雙網，依天數與每日流量選購，高速用完後降速約 128kbps 可持續使用，支援熱點與 ChatGPT。",
    seo_keywords:
      "新加坡eSIM,每日型,M1,Starhub,旅遊eSIM,Jeko eSIM",
    subtitle_by_carrier: {
      [TELECOM]:
        "每日型・M1 / Starhub・高速用完後降速約 128kbps",
    },
    ...
    key_features_by_carrier: {
      [TELECOM]: m1StarhubKeyFeatures("daily"),
    },
    overview_notices_by_carrier: {
      [TELECOM]: {
        fup_notice:
          "依所選方案提供每日高速流量（500MB／1GB／2GB／3GB）。用完後降速至約 128 kbps 可持續使用（每日重置）。M1／Starhub 雙網自動切換。支援熱點與 ChatGPT。",
  ...
  const payloadBase = {
    title: "新加坡 eSIM 每日型  M1 / Starhub",
    subtitle: "M1 / Starhub 雙網・每日流量型・高速用完後約 128kbps",
    handle: HANDLE,
    description:
      "新加坡 eSIM 每日型，走 M1／Starhub 雙網漫遊（香港 IP）。依天數與每日高速流量選購，用完後降速約 128kbps 可持續使用，支援熱點與 ChatGPT／Gemini。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  if (!product) {
    ...
    if (REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-singapore-daily-product.mjs --rebuild",
      );
      return;
    }
  }
  ...
  const s2 = rows.find((r) => r.day === 1 && r.data_amount === "每日 1GB");
  if (s1) {
    console.log(
      `範例 1天500MB: HKD ${s1.price_hkd} → cost NT$${s1.cost_twd} → 售價 NT$${s1.retail_twd}（120%）`,
    );
  }
  if (s2) {
    console.log(
      `範例 1天1GB: HKD ${s2.price_hkd} → cost NT$${s2.cost_twd} → 售價 NT$${s2.retail_twd}（120%）`,
    );
  }
}
