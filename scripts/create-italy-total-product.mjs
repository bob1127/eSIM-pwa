/**
 * 建立「義大利 總量型 eSIM」（單一電信變體）
 *   Iliad / TIM + ← EU 32-Total*-A0（FR IP、orange、高速後 128kbps、100%）
 *   支援 ChatGPT、TikTok、Gemini
 *
 * 用法：
 *   HKD_TO_TWD=4.5 node scripts/create-italy-total-product.mjs
 *   HKD_TO_TWD=4.5 node scripts/create-italy-total-product.mjs --rebuild
 */
import fs from "fs";
...
const HANDLE = "italy-total-esim";
const TITLE = "義大利 總量型 eSIM";
const TELECOM = IT_TELECOM_32;
const PROFIT = 100;
...
const NETWORK_LABEL = "Iliad／TIM／WindTre 4G/5G";
const COVERAGE = "義大利";
...
function collectRows(raw) {
  ...
  for (const p of raw) {
    ...
    if (!/^EU[\s-]*32-Total\d+GB-\d+-A0$/i.test(name)) continue;
...
async function main() {
  ...
  console.log(`  ${TELECOM} ← EU-32-Total*-A0（100%）`);

  const rows = collectRows(await fetchPlans());
  if (!rows.length) throw new Error("找不到 EU-32-Total*-A0");

  const samples = [
    rows.find((r) => r.day === 3 && r.data === "20GB"),
  ...
  const productMeta = {
    ...
    seo_title: "義大利 eSIM 總量型｜Iliad／TIM／WindTre｜Jeko eSIM",
    seo_description:
      "義大利總量型 eSIM，Iliad／TIM／WindTre 4G／5G。可選 1GB～50GB，高速用完後降速約 128kbps。支援 ChatGPT、TikTok、Gemini。建議抵達義大利覆蓋範圍後再啟用。",
    seo_keywords:
      "義大利eSIM,Rome eSIM,Iliad,TIM,WindTre,總量型,旅遊eSIM,Jeko eSIM",
    subtitle_by_carrier: {
      [TELECOM]: "Iliad／TIM／WindTre｜總量高速後約 128kbps｜法國 IP",
    },
    ...
    overview_notices_by_carrier: {
      [TELECOM]: {
        fup_notice:
          "義大利走 Iliad／TIM／WindTre。高速額度用完後降速約 128kbps 可持續使用。出網法國 IP（orange），支援 ChatGPT／TikTok／Gemini。地鐵、古蹟室內與鄉村收訊不保證。",
  ...
  const payloadBase = {
    title: TITLE,
    subtitle: "Iliad／TIM／WindTre｜總量型｜高速後約 128kbps",
    handle: HANDLE,
    description:
      "義大利總量型 eSIM，走 Iliad／TIM／WindTre 4G／5G。可選 1GB／2GB／3GB／5GB／10GB／20GB／30GB／50GB。高速額度用完後降速約 128kbps 可持續使用。支援 ChatGPT、TikTok、Gemini 與熱點。建議抵達義大利覆蓋範圍後再啟用。",
    status: "published",
    discountable: true,
    thumbnail: THUMB,
    images: [{ url: THUMB }],
  ...
  if (!product) {
    ...
    if (!REBUILD) {
      ...
      console.log(
        "重建變體請執行：HKD_TO_TWD=4.5 node scripts/create-italy-total-product.mjs --rebuild",
      );
      return;
    }
