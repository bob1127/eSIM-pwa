...
export async function computeAuthoritativeStoreOrder({
  ...
  // 結帳金額為權威計算：先強制從 DB 載入 boss 後台設定的平台抽成倍率，
  // 確保 applyPartnerB2BMarkup() 用的是最新設定，而非冷啟動 lambda 的舊值。
  await loadB2BMarkupMultiplier({ force: true });

  const listingsRaw = await fetchStoreListings(storeId, { forSaleOnly: true });
  ...
  // 結帳當下再確認主站仍上架，避免快取／延遲同步造成幽靈販售
  const medusaIds = [
    ...new Set(
      listings
        .map((l) => l.medusa_product_id || l.products?.medusa_product_id)
        .filter(Boolean)
        .map(String),
    ),
  ];
  if (medusaIds.length) {
    try {
      const { confirmedMissing } = await probeLiveMedusaProductIds(medusaIds);
      if (confirmedMissing.size) {
        let admin = null;
        try {
          admin = getSupabaseAdminServer();
        } catch {
          admin = null;
        }
        for (const id of confirmedMissing) {
          if (admin) {
            await markMedusaProductOffCatalog(admin, id, { deleted: false });
          }
        }
        listings = listings.filter((l) => {
          const mid = String(
            l.medusa_product_id || l.products?.medusa_product_id || "",
          );
          if (!mid) return true;
          return !confirmedMissing.has(mid);
        });
        if (!listings.length) {
          throw new PricingError(
            "購物車商品已於主站下架，請重新整理後再試",
          );
        }
      }
    } catch (err) {
      ...
      console.warn(
        "[computeAuthoritativeStoreOrder] catalog probe skipped:",
        err?.message || err,
      );
    }
  }
