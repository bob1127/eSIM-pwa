...
import {
  sanitizeItineraryBlock,
  itineraryToHtml,
} from "./partnerBlogItinerary";
...
export const WIDGET_GROUPS = [
  {
    ...
    widgets: [
      ...
      { type: "icon", label: "動態圖示", icon: "star" },
  ...
  },
  {
    ...
    widgets: [
      ...
      { type: "social-post", label: "Instagram 貼文", icon: "photo_camera" },
      { type: "facebook-post", label: "FB 貼文", icon: "thumb_up" },
      { type: "threads-post", label: "Threads 貼文", icon: "alternate_email" },
...
export function sanitizeBlock(raw) {
  ...
  if (type === "itinerary") return sanitizeItineraryBlock(raw);
  ...
  switch (type) {
    ...
    case "text":
      block.props = {
        html: clip(p.html, 20000),
        align: ["left", "center", "right"].includes(p.align) ? p.align : "left",
        font_size: ["15px", "17px", "19px", "22px"].includes(p.font_size)
          ? p.font_size
          : "",
      };
      break;
    ...
    case "icon": {
      ...
      block.props = {
        name: names.includes(p.name) ? p.name : "airplane",
        size,
        label: clip(p.label, 80),
        href: safeHref(p.href, ""),
        align: ["left", "center", "right"].includes(p.align) ? p.align : "center",
      };
      break;
    }
    case "divider":
      ...
      const platform =
        type === "facebook-post"
          ...
          : type === "threads-post"
            ...
            : "instagram";
      block.props = {
        urls: clip(normalizeSocialUrlsField(p.urls, { platform }) || p.urls, 2500),
        nav_urls: clip(
          normalizeSocialUrlsField(p.nav_urls, { platform }) || p.nav_urls,
          4000,
        ),
        video_urls: clip(p.video_urls, 4000),
...
/** 元件 type → 平台過濾（舊 social-post = Instagram） */
export function socialEmbedPlatform(type) {
  if (type === "facebook-post") return "facebook";
  if (type === "threads-post") return "threads";
  return "instagram";
}

export const SOCIAL_EMBED_TYPES = new Set([
  "social-post",
  "facebook-post",
  "threads-post",
]);
...
/**
 * 防呆：客戶貼 iframe／blockquote／雜訊文字時，抽出可辨識的社群網址
 */
export function extractSocialUrlFromPaste(raw) {
  ...
  if (iframeSrc) {
    try {
      ...
      const ig = iframeSrc.match(
        /instagram\.com\/(?:[^/]+\/)?(p|reel|reels|tv)\/([A-Za-z0-9_-]+)/i,
      );
      if (ig) {
        const kind = ig[1] === "reels" ? "reel" : ig[1];
        return `https://www.instagram.com/${kind}/${ig[2]}/`;
      }
      if (/threads\.(net|com)/i.test(iframeSrc)) return iframeSrc;
      if (/facebook\.com|fb\.watch/i.test(iframeSrc)) return iframeSrc;
      return iframeSrc;
    } catch {
      /* fall through */
    }
  ...
  const dataPermalink = input.match(
    /data-(?:instgrm-permalink|text-post-permalink)=["']([^"']+)["']/i,
  )?.[1];
  if (dataPermalink) return dataPermalink;

  const hrefInHtml = input.match(
    /(?:href|permalink)=["'](https?:\/\/[^"']+(?:instagram|threads|facebook|fb\.watch)[^"']*)["']/i,
  )?.[1];
  if (hrefInHtml) return hrefInHtml;

  const bare = input.match(
    /https?:\/\/(?:www\.)?(?:instagram\.com|instagr\.am|threads\.(?:net|com)|facebook\.com|fb\.com|m\.facebook\.com|fb\.watch)[^\s"'<>]*/i,
  )?.[0];
...
/** 把 IG / FB / Threads 貼文網址轉成可嵌入資料 */
export function parseSocialPostUrl(raw, opts = {}) {
  const input = extractSocialUrlFromPaste(raw);
  ...
  const only = opts.platform || null;

  if (host === "instagram.com" || host === "instagr.am") {
    if (only && only !== "instagram") return null;
    const withUser = url.pathname.match(
      /\/([A-Za-z0-9._]+)\/(p|reel|reels|tv)\/([A-Za-z0-9_-]+)/,
    );
    const m = url.pathname.match(/\/(p|reel|reels|tv)\/([A-Za-z0-9_-]+)/);
    if (!m && !withUser) return null;
    const kindRaw = withUser ? withUser[2] : m[1];
    const code = withUser ? withUser[3] : m[2];
    const user =
      withUser && !["p", "reel", "reels", "tv"].includes(withUser[1])
        ? withUser[1]
        : "";
    const kind =
      kindRaw === "p" ? "p" : kindRaw === "tv" ? "tv" : "reel";
    const path = kind === "p" ? "p" : kind === "tv" ? "tv" : "reel";
    return {
      platform: "instagram",
      kind,
      label:
      ...
      profileUrl: user ? `https://www.instagram.com/${user}/` : "",
      // 不用 captioned：隱藏按讚／內文／hashtag，維持簡潔預覽
  ...
  if (
    ...
    if (only && only !== "facebook") return null;
    ...
    const pageMatch = url.pathname.match(
      /^\/(?:pages\/)?([^/]+)\/(?:posts|videos|photos|reel|reels)\//i,
    );
    const pageHandle =
      pageMatch?.[1] &&
      !/^(watch|share|story\.php|permalink\.php)$/i.test(pageMatch[1])
        ? pageMatch[1]
        : "";
    return {
      platform: "facebook",
  ...
  if (host === "threads.net" || host === "threads.com") {
    if (only && only !== "threads") return null;
    const postM = url.pathname.match(/\/(@[^/]+\/post\/)([A-Za-z0-9_-]+)/i);
    const shortM = url.pathname.match(/\/t\/([A-Za-z0-9_-]+)/i);
    if (!postM && !shortM) return null;
    url.hash = "";
    ...
    const path = postM
      ? `/${postM[1]}${postM[2]}`
      : `/t/${shortM[1]}`;
    // 官方嵌入以 threads.com 為準
    const permalink = `https://www.threads.com${path}${path.endsWith("/") ? "" : "/"}`;
    const userM = permalink.match(/threads\.com\/(@[^/]+)/i);
    return {
      platform: "threads",
      kind: "threads",
      label: "Threads",
      permalink,
      profileUrl: userM ? `https://www.threads.com/${userM[1]}/` : permalink,
      embedSrc: "",
      embedMode: "threads",
    };
  }
...
/**
 * 多行貼上：每行可為純網址或整段 iframe／embed 碼
 */
export function parseSocialPostUrls(text, opts = {}) {
  ...
  for (const chunk of chunks) {
    const hit = parseSocialPostUrl(chunk, opts);
    if (!hit) continue;
    const key = hit.permalink || hit.embedSrc;
...
/** 後台儲存前：把 iframe／雜訊正規成乾淨貼文網址（一行一個） */
export function normalizeSocialUrlsField(text, opts = {}) {
  return parseSocialPostUrls(text, opts)
    .map((p) => p.permalink)
    .filter(Boolean)
    .join("\n");
}
...
function blockToHtml(block) {
  ...
  switch (block.type) {
    ...
    case "icon": {
      const caption = p.label ? esc(p.label) : esc(p.name || "icon");
      const inner = p.href
        ? `<a href="${esc(p.href)}">${caption}</a>`
        : caption;
      return `<p style="text-align:${p.align || "center"}">${inner}</p>`;
    }
    case "divider":
      return `<hr />`;
    case "spacer":
    ...
    case "threads-post":
      return parseSocialPostUrls(p.urls, {
        ...
        .map((post) => {
          if (post.embedMode === "threads") {
            return `<blockquote class="text-post-media" data-text-post-permalink="${esc(
              post.permalink,
            )}" data-text-post-version="0"><a href="${esc(
              post.permalink,
            )}">在 Threads 查看</a></blockquote>`;
          }
          return `<p><iframe src="${esc(post.embedSrc)}" width="100%" height="700" loading="lazy" style="border:0;max-width:550px"></iframe></p>`;
        })
        .join("");
    case "map":
      if (!p.query) return "";
