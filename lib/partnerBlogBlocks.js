...
export function parseSocialPostUrls(text, opts = {}) {
  return String(text || "")
    .split(/\n+/)
    .map((line) => parseSocialPostUrl(line.trim(), opts))
    .filter(Boolean)
    .slice(0, 12);
}
    ...
    case "threads-post":
      return parseSocialPostUrls(p.urls, {
        ...
        .map((post) => {
          ...
          return `<p><iframe src="${esc(post.embedSrc)}" width="100%" height="620" loading="lazy" style="border:0;max-width:550px"></iframe></p>`;
        })
        .join("");
    case "map":
      if (!p.query) return "";
