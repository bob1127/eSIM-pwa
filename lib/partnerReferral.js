...
/** 專屬連結預設：成本 × 25%（對外約實付 15%，視方案與折扣而定） */
export const DEFAULT_REFERRAL_RATE = 25;
/** @deprecated 專屬連結已取消達標加碼；保留常數以免舊呼叫端炸掉 */
export const REFERRAL_BONUS_RATE = 30;
/** @deprecated 專屬連結已取消達標加碼 */
...
/** 歸因用落地：官網首頁 + Cookie；可選自動帶折扣碼 */
export function buildReferralUrl(siteUrl, code, { withCoupon = false } = {}) {
  const base = String(siteUrl || "").replace(/\/$/, "");
  const c = normalizeReferralCode(code);
  if (!base || !c) return "";
  const q = new URLSearchParams({ ref: c });
  if (withCoupon) q.set("coupon", c.toUpperCase());
...
/**
 * 專屬連結分潤趴數：以夥伴核准的 referral_rate 為準（無達標加碼）
 */
export function getPartnerReferralRate(partner) {
  const r = Number(partner?.referral_rate);
  if (Number.isFinite(r) && r > 0 && r <= 100) return r;
  return DEFAULT_REFERRAL_RATE;
}

/**
 * @deprecated 達標加碼已停用；回傳結構僅供相容舊 UI
 */
export function resolveReferralTierRates(partner) {
  const baseRate = getPartnerReferralRate(partner);
  return {
    baseRate,
    bonusRate: baseRate,
...
/** 實際分潤趴數（達標加碼已停用，忽略單量） */
export function referralRateForMonthCount(partner, _monthValidCount) {
  return getPartnerReferralRate(partner);
}
