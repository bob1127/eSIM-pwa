...
/** 商品是否含原生 IP／原生線路 */
function productHasNativeIp(product) {
  const hay = `${product?.name || ""} ${product?.description || ""}`;
  if (/原生\s*IP|原生e?SIM|原生線路|Local\s*IP/i.test(hay)) return true;
  return (product?.variants || []).some(isNativeVariant);
}

/**
 * 真・不限速吃到飽（非僅鎖 Mbps 的限速吃到飽）
 * 依商品名／說明／變體標題判斷
 */
function productIsTrueUnlimited(product) {
  const variantHay = (product?.variants || [])
    .map((v) => `${v.title || ""} ${JSON.stringify(v.attributes || {})}`)
    .join(" ");
  const hay = `${product?.name || ""} ${product?.description || ""} ${variantHay}`;
  if (!/吃到飽|無限|unlimited|unlimit/i.test(hay)) return false;
  if (/真[・．.\s]*不限速|不限速吃到飽|true[.\s_-]*unlimited|max\s*speed|high\s*speed/i.test(hay)) {
    // 名稱已標真・不限速／高標速 → 視為真不限速
    if (/真[・．.\s]*不限速|不限速吃到飽/i.test(hay)) return true;
    // high/max speed 但明確寫死低 Mbps（如 僅 1～5Mbps）則不算
    if (/\b([1-5])\s*[-~～到至]\s*([1-9]|1[0-9])?\s*mbps\b/i.test(hay)) return false;
    return true;
  }
  // 吃到飽但標了明顯限速 Mbps（且沒寫真・不限速）→ 不當第一優先
  if (/\d+\s*mbps/i.test(hay) && !/真[・．.\s]*不限速/i.test(hay)) return false;
  return false;
}
...
const NAME_TO_CATEGORY = [
  ...
  [/法國|france|巴黎/i, "france"],
  [/英國|united\s*kingdom|\buk\b|倫敦|愛丁堡/i, "uk"],
  [/奧地利|austria|維也納/i, "austria"],
  [/瑞士|switzerland|蘇黎世|日內瓦/i, "switzerland"],
  [/義大利|意大利|italy|羅馬|米蘭/i, "italy"],
  [/西班牙|spain|馬德里|巴塞隆納/i, "spain"],
  [/土耳其|turkey|turkiye|伊斯坦堡/i, "turkey"],
  [/歐洲|europe/i, "europe"],
  [/美國|加拿大|usa|america/i, "usa"],
  [/紐澳|澳紐/i, "anz"],
  [/紐西蘭|new\s?zealand/i, "newzealand"],
  [/澳洲|australia/i, "australia"],
...
const COUNTRY_ALIASES = [
  ...
  },
  {
    label: "法國",
    slugs: ["france"],
    keys: ["法國", "france", "巴黎", "里昂", "尼斯"],
  },
  {
    label: "英國",
    slugs: ["uk"],
    keys: ["英國", "uk", "london", "倫敦", "愛丁堡", "曼徹斯特"],
  },
  {
    label: "奧地利",
    slugs: ["austria"],
    keys: ["奧地利", "austria", "vienna", "維也納", "薩爾斯堡"],
  },
  {
    label: "瑞士",
    slugs: ["switzerland"],
    keys: ["瑞士", "switzerland", "zurich", "蘇黎世", "日內瓦", "伯恩", "琉森"],
  },
  {
    label: "義大利",
    slugs: ["italy"],
    keys: ["義大利", "意大利", "italy", "rome", "羅馬", "米蘭", "佛羅倫斯", "威尼斯"],
  },
  {
    label: "西班牙",
    slugs: ["spain"],
    keys: ["西班牙", "spain", "madrid", "barcelona", "馬德里", "巴塞隆納"],
  },
  {
    label: "土耳其",
    slugs: ["turkey"],
    keys: ["土耳其", "turkey", "turkiye", "伊斯坦堡", "安塔利亞"],
  },
  {
    label: "歐洲",
    slugs: ["europe"],
...
/** 關鍵字相關者先留下，再：真・不限速 → 原生 IP → HOT SALE → 分數 */
function pickRelevantProducts(all, queryText, limit = 3) {
  ...
  const scored = pool
    .map((p) => ({
      ...
      trueUnlim: productIsTrueUnlimited(p),
      native: productHasNativeIp(p),
      hot: Array.isArray(p.hotSaleTelecoms) && p.hotSaleTelecoms.length > 0,
    }))
    .filter((x) => x.score > 0)
    .sort((a, b) => {
      if (a.trueUnlim !== b.trueUnlim) return a.trueUnlim ? -1 : 1;
...
function formatProductKnowledgeText(items, queryText = "") {
  const countries = detectCountriesFromQuery(queryText);
  ...
  for (const p of items) {
    ...
    const tags = [];
    if (productIsTrueUnlimited(p)) tags.push("真・不限速吃到飽");
    if (productHasNativeIp(p)) tags.push("原生IP");
    if (p.hotSaleTelecoms?.length > 0) {
      tags.push(`HOT SALE：${p.hotSaleTelecoms.join("、")}`);
    }
    const tagNote = tags.length ? `（標籤：${tags.join("／")}）` : "";
    lines.push(`\n▸ ${p.name}${tagNote}`);
    // 說明截短，減少 prompt token
    if (p.description) {
      const desc = String(p.description).replace(/\s+/g, " ").trim().slice(0, 160);
      if (desc) lines.push(`  說明：${desc}${p.description.length > 160 ? "…" : ""}`);
    }
    lines.push(`  購買連結：${url}`);
    if (p.variants?.length) {
      const shown = p.variants.slice(0, 6);
      lines.push(`  方案（共 ${p.variants.length} 種，真・不限速／原生／HOT SALE 優先）：`);
      for (const v of shown) {
        const telecom = v.attributes?.telecom || "";
        const hot = isHotSaleTelecom(p.hotSaleTelecoms, telecom);
        const native = isNativeVariant(v);
        ...
        const prefix = [
          ...
          .filter(Boolean)
          .join(" ");
        lines.push(`    - ${prefix ? `${prefix} ` : ""}${label}`);
      }
      if (p.variants.length > 6) lines.push(`    ...詳見購買連結`);
    }
  }
  lines.push(
    "\n【重要｜禁止捏造商品】只能引用上方「▸」商品與連結；聊天室會顯示商品卡。" +
      "禁止推薦未列出方案或競品電商。" +
      "【推薦順序】①真・不限速吃到飽 → ②原生IP；同順位再套 HOT SALE。" +
      "【用量緩衝】第1優先吃到飽；第2總量須達天數×每日下限（輕1.5／社群2.5／視訊3.5／影音5 GB/日）。" +
      "禁止剛好均攤；價格以資料庫為準。",
  );
  return lines.join("\n");
}

function mapProductsToCards(items) {
  return (items || []).map((p) => {
...
export async function fetchProductKnowledge(queryText = "") {
  const bundle = await fetchProductBundle(queryText);
...
/**
 * 一次載入＋篩選，同時回知識庫文字與卡片（避免重複查 Medusa／Supabase）
 */
export async function fetchProductBundle(queryText = "") {
  ...
  return {
    knowledge: formatProductKnowledgeText(items, queryText),
    cards: queryText ? mapProductsToCards(items) : [],
    items,
  };
}
