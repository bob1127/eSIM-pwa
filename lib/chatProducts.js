...
function isNativeVariant(v) {
  ...
  if (
    /(日本|韓國|泰國|中國|本地)\s*IP/i.test(hay) &&
    !/新加坡|SG\b/i.test(hay)
  ) {
    return true;
  }
...
// ── 國家 → category slug 對照 ───────────────────────────────────────────
// 注意：Medusa 泰國分類 handle 目前為拼字 "tailand"（非 thailand）
const NAME_TO_CATEGORY = [
  [/日本|japan/i, "japan"],
  [/韓國|korea|首爾/i, "korea"],
  [/泰國|thailand|曼谷|tailand/i, "tailand"],
...
function guessCategoryFromHandle(handle = "") {
  ...
  for (const slug of HANDLE_SLUGS) {
    if (h === slug || h.startsWith(`${slug}-`) || h.includes(`-${slug}`)) {
      // thailand-* handle 對應 Medusa 分類 tailand
      if (slug === "thailand") return "tailand";
      return slug;
    }
  }
...
export async function fetchProductKnowledge(queryText = "") {
  ...
  // 有指定國家但對不到：不要改塞其他國家商品，避免 AI 說「沒有」又亂推競品
  if (queryText && !items.length) {
    if (countries.length) {
      return (
        `【最新商品資料庫】\n` +
        `（目前資料庫未找到「${countries.map((c) => c.label).join("、")}」相符方案。` +
        `請引導使用者至 /product 瀏覽，或轉真人客服；禁止推薦其他電信／eSIM 電商網站。）`
      );
    }
    items = all.slice(0, 3);
  }
  ...
  lines.push(
    "\n【重要】回答時只引用上方商品資料與購買連結；聊天室會顯示商品卡。" +
