...
async function loadFromMedusa() {
  ...
  return all.map((p) => {
    const f = formatMedusaProductForPartner(p);
    return {
      name: f.name,
...
export async function fetchProductKnowledge(queryText = "") {
  ...
  // 有查詢文字 → 評分篩選；無查詢文字 → 回傳全部
  let items = all;
  if (queryText) {
    const scored = all
      .map((p) => ({ p, score: scoreProduct(p, queryText) }))
      .sort((a, b) => b.score - a.score);

    // 分數 > 0 取前 2，否則退回全部（至多 3 個）
    const relevant = scored.filter((x) => x.score > 0).slice(0, 2);
    items = relevant.length ? relevant.map((x) => x.p) : all.slice(0, 3);
  }
  ...
  for (const p of items) {
    ...
    if (p.variants?.length) {
      ...
      lines.push(`  方案（共 ${p.variants.length} 種，代表方案）：`);
  ...
  lines.push(
    "\n【重要】回答時只引用上方商品資料；連結直接貼購買連結；價格以資料庫為準，禁止臆測。"
  );
  return lines.join("\n");
}
...
export async function fetchProductCards(queryText = "") {
  ...
  if (queryText) {
    ...
    const relevant = scored.filter((x) => x.score > 0).slice(0, 3);
    items = relevant.length ? relevant.map((x) => x.p) : [];
  }
