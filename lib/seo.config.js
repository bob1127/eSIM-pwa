...
export function resolveProductCategoryBreadcrumbLabel(product, categoryHandle) {
  ...
  if (/泰國|thailand/i.test(name) || cat === "thailand" || cat === "th")
    return "泰國";
  if (/馬來|malaysia/i.test(name) || cat === "malaysia" || cat === "my")
    return "馬來西亞";
  if (/新加坡|singapore/i.test(name) || cat === "singapore" || cat === "sg")
    return "新加坡";
  if (/美國|usa|united.?states/i.test(name) || cat === "usa" || cat === "us")
    return "美國";
  if (/歐洲|europe/i.test(name) || cat === "europe" || cat === "eu")
    return "歐洲";
  if (/全球|global|worldwide/i.test(name) || cat === "global") return "全球";
