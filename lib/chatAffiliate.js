...
const AFFILIATE_INTENT =
  /klook|kkday|可購|客路|飯店|住宿|酒店|hotel|民宿|訂房|過夜住宿|門票|票券|通行證|一日券|入場|pass|樂園|環球|迪士尼|展望台|景點票|體驗|活動票|一日遊|半日遊|導覽|自助餐|美食|餐券|交通票|交通卡|地鐵卡|火車|鐵道|rail|\bjr\b|周遊券|t-?money|ktx|西瓜卡|ic\s*卡|機場交通|接駁/i;

const HOTEL_INTENT = /飯店|住宿|酒店|hotel|民宿|訂房|過夜|住哪/i;
const TICKET_INTENT =
  /門票|票券|通行證|一日券|入場|pass|樂園|環球|迪士尼|展望|景點|體驗|活動|一日遊|半日遊|導覽|自助餐|美食|餐券/i;
const TRANSPORT_INTENT =
  /交通票|交通卡|地鐵|火車|鐵道|rail|\bjr\b|周遊券|t-?money|ktx|西瓜卡|ic\s*卡|機場交通|接駁|包車|船票|渡輪|ferry|城巴|開篷巴士/i;
...
function buildCatalog() {
  ...
  for (const h of KLOOK_HOTELS || []) {
    list.push(normalizeItem({ ...h, kind: "hotel" }, h.partner || "klook"));
  }
  for (const t of KLOOK_TH_TICKETS || []) {
    list.push(normalizeItem(t, "klook"));
  }
  for (const t of KLOOK_TH_TRANSPORT || []) {
    list.push(normalizeItem(t, "klook"));
  }
  ...
  for (const t of KLOOK_KR_TICKETS || []) {
    list.push(normalizeItem(t, "klook"));
  }
  for (const t of KLOOK_KR_TRANSPORT || []) {
    list.push(normalizeItem(t, "klook"));
  }
  for (const t of KLOOK_CN_TICKETS || []) {
    list.push(normalizeItem(t, "klook"));
  }
  for (const t of KLOOK_HK_TICKETS || []) {
    list.push(normalizeItem(t, "klook"));
  }
...
const REGION_HINTS = [
  ...
  {
    re: /泰國|曼谷|普吉|清邁|芭達雅|thailand|bangkok|phuket|chiang\s*mai|pattaya/,
    countryIds: ["thailand"],
    boost: 18,
  },
  {
  ...
  {
    re: /香港|港澳|澳門|威尼斯人|噴射飛航|港珠澳|維多利亞港|海洋公園|挪亞方舟|hong\s*kong|macau|macao/,
    countryIds: ["hongkong"],
    boost: 18,
  },
  {
    re: /中國|大陸|上海|北京|廣州|深圳|長隆|環球影城|長城|china|shanghai|beijing|guangzhou|shenzhen/,
    countryIds: ["china"],
    boost: 18,
  },
  {
    re: /越南|河內|胡志明|峴港|會安|下龍|富國|vietnam|hanoi|saigon|da\s*nang|hoi\s*an|phu\s*quoc/,
    countryIds: ["vietnam"],
    boost: 18,
  },
];
...
function isTransportItem(item) {
  const blob = [item.title, item.subtitle, item.category, item.badge]
    .join(" ")
    .toLowerCase();
  return /交通|地鐵|火車|鐵道|t-?money|ktx|通票|鐵道周遊|rail|\bjr\b|周遊|西瓜|ic\s*卡|機場交通|包車|船票|渡輪|ferry|城巴|開篷/.test(
    blob
  );
}
