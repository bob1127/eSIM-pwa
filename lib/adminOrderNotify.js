...
import { resolveAccountingChannel } from "./accountingSheet";
...
function buildCopy(payload) {
  ...
  const amount = fmt(payload.amount);
  ...
  const subject = `【Jeko 新訂單】${amount} · ${channel} · #${shortId}`;
  const bodyLines = [
    `新訂單已付款`,
    ``,
    `金額：${amount}`,
  ...
  if (payload.tradeNo) bodyLines.push(`金流單號：${payload.tradeNo}`);
  bodyLines.push(``, `Boss 後台：${bossUrl}`);
