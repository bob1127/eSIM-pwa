...
export function getAlertThresholds() {
  return {
    ...
    // 預設 8 小時：偏低狀態下一天大約可提醒 2～3 次
    cooldownHours: Number(process.env.TRAFFIC_ALERT_COOLDOWN_HOURS || 8),
  };
}
...
async function checkOneTarget(supabaseAdmin, target, table) {
  ...
  const alertData = {
    ...
    sku: usage.data.sku || null,
    planId: usage.data.planId || null,
    ruleDesc: usage.data.ruleDesc || null,
    specialDesc: usage.data.specialDesc || null,
    speedDesc: usage.data.speedDesc || null,
  ...
  // 組文案前先解析方案類型，寫入 log 方便確認推對 FUP／固定流量
  try {
    const profile = resolveTrafficPlanProfile(messageTarget);
