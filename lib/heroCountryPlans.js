...
function formatMedusaProduct(product, categoryHandle) {
  ...
  const telecom =
    meta.telecom || meta.carrier || meta.operator || variant?.metadata?.telecom || "";
  const planId =
    meta.plan_id ||
    meta.planId ||
    variant?.metadata?.plan_id ||
    variant?.metadata?.planId ||
    null;
  ...
  return {
    ...
    telecom: telecom ? String(telecom) : "",
    /** 卡片簡短規格列 */
    specLabel: [telecom, daysLabel ? `${daysLabel}`.replace(/日$/, "日") : "", dataLabel]
      .filter(Boolean)
      .join(" · "),
  };
}
...
export function buildHeroCountries(
  ...
  filterLeafCategoriesForNav(categories).forEach((cat) => {
