...
/**
 * 統一前台文章 shape（僅夥伴來源）
 * 編輯者名稱優先用當前賣場「分店顯示名稱」，避免改店名後舊 author_name 殘留。
 */
export function normalizeStoreBlogRow(row, store = null) {
  if (!row) return null;
  const liveBrand =
    String(store?.store_name || "").trim() ||
    String(store?.footer_company_name || "").trim() ||
    "";
  const editorFallback = liveBrand || "夥伴編輯";
  ...
  return {
    ...
    image: ogImage,
    contentHtml,
    authorName: liveBrand || row.author_name || editorFallback,
    editorName: liveBrand || row.author_name || editorFallback,
