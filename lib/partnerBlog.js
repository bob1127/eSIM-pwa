...
export async function pingPartnerBlogRevalidate({ token, slug, oldSlug } = {}) {
  ...
  try {
    await fetch("/api/partner/revalidate-blog", {
      ...
      body: JSON.stringify({ slug, oldSlug: oldSlug || "" }),
    });
  } catch {
    /* ISR 刷新失敗不擋發布 */
  }
...
export function normalizeStoreBlogRow(row, store = null) {
  ...
  return {
    ...
    slug: row.slug,
    title: row.title,
    excerpt: (row.excerpt || row.meta_description || "").slice(0, 200),
