...
export const CF_IMG_BOOTSTRAP = `(function () {
  try {
    ...
    function rewrite(img) {
      ...
      img.setAttribute("data-cf-orig", src);
      img.setAttribute("src", next);
      if (img.getAttribute("srcset")) img.removeAttribute("srcset");
      // 額度用完／轉換失敗：回退原圖（R2 外鏈 onerror=redirect 不一定生效）
      if (!img.getAttribute("data-cf-onerror")) {
        img.setAttribute("data-cf-onerror", "1");
        img.addEventListener("error", function onCfFail() {
          var orig = img.getAttribute("data-cf-orig");
          if (!orig || img.getAttribute("src") === orig) return;
