...
/**
 * 商城 ISR：列出可預渲染的實體商品 handle
 * @returns {Promise<string[]>}
 */
export async function listShopProductHandles({ limit = 80 } = {}) {
  const backendUrl = getMedusaStoreApiUrl();
  const key = getMedusaPublishableKey();
  if (!key) return [];
  ...
  const headers = { "x-publishable-api-key": key };
  const query = new URLSearchParams({
    limit: String(Math.min(Math.max(limit, 24), 100)),
    offset: "0",
    order: "-created_at",
    fields:
      "id,handle,title,thumbnail,+metadata,*images,*type,*categories,*collection",
  });
