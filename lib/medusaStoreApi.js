...
export async function formatMedusaProductForPartner(
  ...
  if (liveCost) {
    ...
    // 選品管理商品池：只用本地 metadata／估算，毫秒級算完
    formattedVariants = variants.map((v) => {
      const api_b2b_price = resolveApiWholesalePrice(v, meta);
      return {
        medusa_variant_id: v.id,
        sku: v.sku,
        title: v.title,
...
export async function fetchMedusaStoreProductSummaries() {
  ...
  while (true) {
    const query = new URLSearchParams({
      limit: String(limit),
      offset: String(offset),
      fields: "id,title,handle,thumbnail,description,created_at,+metadata",
    });
