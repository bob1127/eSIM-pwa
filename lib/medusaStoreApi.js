 ...
 */
/** 全站搜尋用：只抓標題／描述／分類，不展開變體（比 summaries 快很多） */
export async function fetchMedusaSearchProductIndex() {
  const products = await fetchAllVisibleStoreProducts(
    "id,title,handle,description,thumbnail,+metadata,*categories",
  );
  return products.map((product) => {
    ...
    return {
      ...
      image_url: resolveMedusaImageUrl(product.thumbnail),
      categoryHandle: cats[0]?.handle || "",
      categories: cats.map((c) => c?.handle || c?.name || "").filter(Boolean),
    };
  });
}
    ...
    await Promise.all(
      chunk.map(async (id) => {
        try {
          ...
          map[String(product.id)] = resolveHotSaleTelecomsForProduct(product);
        } catch {
