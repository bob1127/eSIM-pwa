...
function parseExpiryTimestamp(expiresAt) {
...
export const DEFAULT_TRAFFIC_ALERT_COPY = {
  ...
  lineExtra: "※ 監測時間 {{checkedAt}}；用量更新通常需間隔 30–60 分鐘",
  /** 高速額度＋降速吃到飽 */
  fupTitle: "⚠️ 高速流量即將用完",
  fupBody:
    "{{productName}} 高速剩餘 {{remaining}}{{totalPart}}。用完後仍可上網，將維持約 {{throttleSpeed}} 吃到飽。降速後速度依方案與現地網路而定。",
  fupLineExtra:
    "※ 高速額度約 {{highSpeedQuota}}；監測時間 {{checkedAt}}；用量更新通常需間隔 30–60 分鐘",
  /** 純吃到飽／無剩餘 MB：提醒使用期限 */
  expiryTitle: "⏰ eSIM 使用期限提醒",
  expiryBody:
    "{{productName}} 為吃到飽方案，請留意使用期限：效期至 {{expiresAt}}（約剩 {{hoursLeft}}）。請於期限內使用完畢。",
  expiryLineExtra:
    "※ 已用約 {{used}}；監測時間 {{checkedAt}}；吃到飽以使用期限為主，無「剩餘流量」概念",
  linkPath: "/data-query/",
};
...
export function normalizeTrafficAlertCopy(raw) {
  ...
  const expiryTitle = asString(src.expiryTitle, d.expiryTitle).trim();
  const expiryBody = asString(src.expiryBody, d.expiryBody).trim();
  const expiryLineExtra = asString(src.expiryLineExtra, d.expiryLineExtra).trim();
  ...
  if (!expiryTitle || !expiryBody) {
    return { ok: false, message: "使用期限：標題與內文不可空白" };
  }
  ...
  for (const [label, val, max] of [
    ...
    ["使用期限標題", expiryTitle, 80],
    ["使用期限內文", expiryBody, 500],
    ["使用期限 LINE 補充", expiryLineExtra, 300],
  ]) {
...
export function buildTrafficAlertVars(target = {}) {
  ...
  const used = formatMb(target.usedMb) || "—";
  ...
  const profile =
    target.planProfile ||
    (target.planKind === "expiry" || target.alertKind === "expiry"
  ...
  const endMs = parseExpiryTimestamp(target.expiresAt);
  let expiresAtLabel = "";
  let hoursLeftLabel = "";
  if (endMs != null) {
    const d = new Date(endMs);
    expiresAtLabel = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}-${String(d.getDate()).padStart(2, "0")} ${String(d.getHours()).padStart(2, "0")}:${String(d.getMinutes()).padStart(2, "0")}`;
    const hoursLeft = Math.max(0, (endMs - Date.now()) / 3600000);
    if (hoursLeft >= 48) {
      hoursLeftLabel = `${Math.round(hoursLeft / 24)} 天`;
    } else if (hoursLeft >= 1) {
      hoursLeftLabel = `${Math.max(1, Math.round(hoursLeft))} 小時`;
    } else {
      hoursLeftLabel = `${Math.max(1, Math.round(hoursLeft * 60))} 分鐘`;
    }
  }
  ...
  return {
    ...
    throttleSpeed,
    checkedAt,
    expiresAt: expiresAtLabel || String(target.expiresAt || "").slice(0, 16),
    hoursLeft: hoursLeftLabel || "不久",
...
function pickTemplates(copy, kind) {
  if (kind === "expiry") {
    return {
      title: copy.expiryTitle || copy.title,
      body: copy.expiryBody || copy.body,
      lineExtra: copy.expiryLineExtra || copy.lineExtra,
    };
  }
  if (kind === "fup") {
    return {
...
export async function describeTrafficAlertCopy() {
  ...
  return {
    copy: {
      ...
      expiryTitle: copy.expiryTitle,
      expiryBody: copy.expiryBody,
      expiryLineExtra: copy.expiryLineExtra,
...
function resolveKind(target) {
  if (
    target.alertKind === "expiry" ||
    target.planKind === "expiry"
  ) {
    return "expiry";
  }
