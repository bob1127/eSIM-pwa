import { stripWpReadMore } from "@/lib/stripHtml";

function cleanRenderedField(field) {
  if (!field || typeof field !== "object") return field;
  if (typeof field.rendered !== "string") return field;
  return { ...field, rendered: stripWpReadMore(field.rendered) };
}

/** 去掉 WP 「閱讀更多」留下的 [&hellip;] / &hellip; */
export function cleanWpPostRendered(post) {
  if (!post) return post;
  return {
    ...post,
    title: cleanRenderedField(post.title),
    excerpt: cleanRenderedField(post.excerpt),
    content: cleanRenderedField(post.content),
  };
}
...
export async function fetchWpPosts(query = {}) {
  ...
  return data.map(cleanWpPostRendered);
}
...
export async function fetchAllWpPosts(options = {}) {
  ...
  while (page <= totalPages && page <= maxPages) {
    ...
    all.push(...data.map(cleanWpPostRendered));
...
export function buildBlogCategoryMaps(categories) {
  ...
  const articleRoot = list.find((c) => c.slug === "article");
  const knowledgeRoot = list.find((c) => c.slug === "knowlage");
  ...
  const categoriesById = Object.fromEntries(
    list.map((cat) => [cat.id, cat]),
  );
  const childrenByParentId = buildChildrenByParent(list);
...
export function slimWpPostCard(post) {
  ...
  return {
    ...
    slug: post.slug,
    date: post.date,
    title: { rendered: stripWpReadMore(post.title?.rendered || "") },
...
export function slimWpPostForPage(post) {
  ...
  return {
    ...
    jetpack_featured_media_url: post.jetpack_featured_media_url
      ...
      : null,
    title: { rendered: stripWpReadMore(post.title?.rendered || "") },
    excerpt: { rendered: stripWpReadMore(post.excerpt?.rendered || "") },
    content: { rendered: stripWpReadMore(post.content?.rendered || "") },
    ...
    ...(post.partnerContribution
      ? {
          ...
          partnerStoreName: post.partnerStoreName || null,
          partnerStoreDomain: post.partnerStoreDomain || null,
          partnerAuthorName: post.partnerAuthorName || null,
        }
      : {}),
    _embedded: {
      ...(mediaUrl
