...
export const CARD_DESIGN_TYPES = new Set([
  "social-post",
  "facebook-post",
  "threads-post",
  "products",
...
export function sanitizeDesign(p = {}, type = "") {
  const width = ["full", "auto", "75", "50", "33", "custom", "read"].includes(
    p.width,
  )
    ? p.width
    : "";
...
export function photoWallFrameStyle(p = {}) {
  ...
  // 舊預設 custom/720：改視為欄寬滿版；要固定 720 請選「閱讀寬」
  const width =
    p.width === "custom" && Number(p.width_px) === PHOTO_WALL_READ_WIDTH
      ? "full"
      : p.width || "full";
  if (width === "75") {
    frame.width = "75%";
    ...
    const px = Math.min(
      1200,
      Math.max(160, Number(p.width_px) || PHOTO_WALL_READ_WIDTH),
    );
    frame.width = `${px}px`;
  } else {
    // full / auto：欄寬滿版
...
/** 文字流元件：不可 overflow 裁切（會把段落切掉） */
const SKIP_CLIP = new Set([
  "social-post",
  "facebook-post",
  "threads-post",
  "photo-wall",
...
const SKIP_RADIUS_SHELL = new Set([
  "social-post",
  "facebook-post",
  "threads-post",
  "products",
...
/** 強制滿版、忽略固定高度（避免段落被切） */
const FORCE_FULL_WIDTH = new Set([
  "text",
  "heading",
  "quote",
  "alert",
...
export function designShellStyle(type, p = {}) {
  ...
  const shellP =
    type === "photo-wall"
      ? { ...p, width: "full" }
      : FORCE_FULL_WIDTH.has(type)
        ? { ...p, width: "full", height_mode: undefined }
        : p;
  const style = {
    maxWidth: "100%",
  ...
  const minH = FORCE_FULL_WIDTH.has(type) ? undefined : minHeightCss(p);
  ...
  if (r != null && !skipShellRadius) {
    ...
    // 只有真正需要圓角裁切的媒體／卡片才 hidden；radius=0 也不該裁切
    if (r > 0 && !SKIP_CLIP.has(type)) style.overflow = "hidden";
  }
  if (p.border_w) {
