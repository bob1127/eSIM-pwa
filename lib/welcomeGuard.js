...
import {
  WELCOME_SOURCE,
  isPendingWelcomeEmail,
} from "./memberWelcomeBenefit";
...
export async function assertWelcomeRedeemAllowed(
  ...
  {
    couponRow = null,
    isPublicNewMemberCode = false,
    guestLineVerify = false,
  } = {},
) {
  ...
  if (!lineUserId) {
    const msg = guestLineVerify
      ? "請先完成 LINE 身分驗證，並確認已加入官方帳號好友"
      : "請先「連結 LINE」並加入官方帳號後，才能使用新會員 50 元優惠（Google／Email 會員不必改用 LINE 登入）";
    return {
      ok: false,
      status: 403,
      need_line_friend: true,
      need_line_verify: guestLineVerify,
      line_oa_url: lineOaUrl,
      error: msg,
      code: guestLineVerify ? "NEED_LINE_VERIFY" : "NEED_LINE_BIND",
  ...
  if (couponRow) {
    ...
    const guestLineOk =
      guestLineVerify &&
      boundLine &&
      boundLine === String(lineUserId) &&
      (!memberEmail || isPendingWelcomeEmail(ownerEmail));
