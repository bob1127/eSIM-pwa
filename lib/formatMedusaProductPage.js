...
function formatProductRecord(product) {
  ...
  return {
    ...
    variations: (product.variants || []).map(mapVariant),
  };
}
...
export async function fetchFormattedMedusaProductPage(query = {}) {
  ...
  const params = new URLSearchParams({
    fields:
      "+metadata,*variants,*variants.prices,*variants.calculated_price,*variants.options,*variants.options.option,*images",
  });
...
/** 夥伴加價：覆寫 variations[].price */
export function applyPartnerMarkupToVariations(
  variations,
  { markupRate = 0, customPrices = {} } = {},
) {
  ...
  return (variations || []).map((v) => {
    ...
    const partnerPrice =
      custom !== undefined && custom !== null
        ...
        : Math.round(retail * (1 + rate / 100));
    return {
      ...v,
      price: partnerPrice,
      original_price: retail > partnerPrice ? retail : v.original_price || retail,
      retail_price: retail,
    };
  });
}
