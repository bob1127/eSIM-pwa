...
} from "./resolveProductListingImage";
...
import { resolveHotSaleTelecomsForProduct } from "./productHotSale";
...
export function mapMedusaStoreVariant(v, productMetadata = {}) {
  return mapVariant(v, productMetadata);
}
...
function mapVariant(v, productMetadata = {}) {
  ...
  return {
    ...
    special_desc: v.metadata?.special_desc || "",
...
function formatProductRecord(product) {
  ...
  // 與主站 product/[category]/[slug] 相同：特色圖 + 共用教學輪播
  const image_url = resolveProductListingImage(product.thumbnail, imageCtx);
  ...
  return {
    product: {
      ...
      carrier_specs_by_carrier:
        ...
        {},
      hot_sale_telecoms: resolveHotSaleTelecomsForProduct(product),
...
export async function fetchCategoryComparablePlans({
  ...
  for (const p of products) {
    ...
    const hotSaleTelecoms = resolveHotSaleTelecomsForProduct(p);
    const carrierSpecs =
      parseCarrierSpecsByCarrier(p.metadata?.carrier_specs_by_carrier) || {};
    const variations = (p.variants || []).map((v) =>
      mapVariant(v, p.metadata || {}),
    );
    ...
    for (const v of variations) {
      ...
      const isNativeIp = isNativeIpPlan(
        { ...v, productName: p.title || "", carrierSpec },
        {
          ...p.metadata,
          name: p.title,
