...
/**
 * 測試／假資料訂單（應從主站報表排除）。
 * 對齊 accountingSheet／adminOrderNotify：【測試購買】、tx_mock、e2e 信箱等。
 */
export function isTestMedusaOrder(order) {
  const email = String(
    order?.email || order?.metadata?.checkout_email || "",
  ).toLowerCase();
  if (
    email.endsWith("@jeko-esim.local") ||
    email.endsWith("@example.com") ||
    email.includes("e2e-linepay") ||
    email.includes("test@") ||
    /^test[-+.]/i.test(email)
  ) {
    return true;
  ...
  const metaBlob = [
    ...
    meta.linepay_order_no,
    meta.auth_provider,
    meta.checkout_note,
  ]
    .map((v) => String(v || ""))
    .join(" ");
  if (/tx_mock|test-jeko|test-combined|test-local-notify|cursor 自動測試/i.test(metaBlob)) {
    return true;
  }
  ...
  for (const it of items) {
    const name = String(
      it.product_title || it.title || it.variant_title || it.variant_sku || "",
    );
    if (/測試購買|【測試|test purchase/i.test(name)) return true;
    const sku = String(it.variant_sku || it.sku || "");
    if (/^test[-_]/i.test(sku)) return true;
  }
...
export function buildMainSiteSalesReport(medusaOrders = [], opts = {}) {
  ...
  for (const o of medusaOrders || []) {
    ...
    if (isTestMedusaOrder(o)) continue;
