...
/**
 * LINE Flex 原生 button 幾乎無圓角；改用可點擊的圓角 box 模擬 CTA
 */
export function buildRoundedFlexCta({
  label,
  uri,
  ...
  return {
    ...
    action: {
      type: "uri",
      label: clip(label, 40) || "查看詳情",
      uri,
    },
    contents: [
      {
        type: "text",
        text: clip(label, 20) || "查看詳情",
        color: textColor,
        align: "center",
        weight: "bold",
        size: "sm",
  ...
  return {
    ...
    contents: [
      buildRoundedFlexCta({
        label: clip(label, 20) || "查看詳情",
        uri: link,
        backgroundColor: style.buttonColor || "#06C755",
      }),
    ],
  };
}
