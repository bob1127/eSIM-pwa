...
export const NATIVE_ESIM_REFUND_MESSAGE =
  "原生 eSIM（原生 IP／本地線路）一經出貨即不適用退款與售後爭議退款申請。如有連線問題請洽客服協助排查，恕無法受理退款。";
...
/** 訂單是否含原生 eSIM（原生 IP／本地線路） */
export function isNativeEsimOrder(order) {
  if (!order) return false;
  if (
    order.is_native === true ||
    order.native_esim === true ||
    order?.metadata?.is_native === true ||
    order?.metadata?.native_esim === true
  ) {
    return true;
  }
  ...
  for (const item of items) {
    if (!item || item._payment_demo) continue;
    if (
      item.is_native === true ||
      item.native_esim === true ||
      item?.metadata?.is_native === true ||
      item?.metadata?.native_esim === true
    ) {
      return true;
    }
    const plan = {
      ...
      isNativeIp: item.isNativeIp === true,
      attributes: {
        ...(item.attributes || {}),
        telecom: item.telecom || item.attributes?.telecom,
        ip_type: item.ip_type || item.attributes?.ip_type,
        route_type: item.route_type || item.attributes?.route_type,
      },
    };
    const meta = {
      ...
      native_esim: item.native_esim,
      metadata: item.metadata || {},
    };
    if (isNativeIpPlan(plan, meta)) return true;
  }

  const summary = orderItemSummary(order);
  return isNativeIpPlan(
    { productName: summary, attributes: {} },
...
export function getRefundEligibility(order) {
  ...
  if (isNativeEsimOrder(order)) {
    return {
      ...
      label: "無法申請退款",
      hint: NATIVE_ESIM_REFUND_MESSAGE,
    };
  }
  ...
  if (activated) {
    if (age <= REFUND_DISPUTE_DAYS) {
      return {
        ...
        requestType: "dispute",
        label: "申請售後／爭議處理",
        hint: "系統偵測此 eSIM 已開通／已使用，請上傳設定畫面或錯誤截圖，由客服個案審核（不保證退款）。",
  ...
  if (age <= REFUND_FULL_DAYS) {
    return {
      ...
      label: "申請退款（未開通）",
      hint: `購買後 ${REFUND_FULL_DAYS} 日內且尚未掃描 QR Code，可申請全額退款。系統會向供應商自動查核開通狀態。`,
      activationSource: "auto",
    };
  }
...
export function refundColumnLabel(order) {
  ...
  if (el.code === "NATIVE_ESIM") return "不可退";
...
export function shouldShowRefundDetail(order) {
  ...
  if (latest && ["pending", "approved", "rejected"].includes(latest.status)) {
    return true;
  }
  return false;
}
...
export function refundStatusLabel(order) {
  ...
  if (status === "refunded" || latest?.status === "approved") {
    return {
      label: "已退款",
      color: "bg-[#F1F3F7] text-[#5B6570] border-[#E3E7EE]",
    };
  }
  if (status === "refund_pending" || latest?.status === "pending") {
    return {
      label: "退款審核中",
      color: "bg-[#FFF6C8] text-[#1E4AD1] border-[#FADE2B]",
    };
  }
  if (latest?.status === "rejected") {
    return {
      label: "退款未通過",
      color: "bg-[#FFF0D6] text-[#1E4AD1] border-[#F2CC40]",
    };
  }
  return null;
}

/** 可計業績／分潤：已完成或待付款（不含取消、退款） */
...
export function orderItemSummary(order) {
  ...
  const first = items.find((i) => i && !i._payment_demo) || items[0];
