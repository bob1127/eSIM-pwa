...
/**
 * 加速提領已匯款、且落在「成交月起～匯款月止」者，自月結應付扣減，避免重複匯款。
 */
 ...
 * @param {object[]} [withdrawals] - partner_withdrawal_requests rows
 */
export function buildPartnerSettlementStatement(
  ...
  const payoutEnd = new Date(`${schedule.payoutDate}T23:59:59.999+08:00`);
  const deductedWithdrawals = filterWithdrawalsForSettlementPeriod(
    withdrawals,
    { start, payoutEnd },
  );
  const withdrawnPaid = deductedWithdrawals.reduce(
    (s, w) => s + Math.round(Number(w.amount) || 0),
    0,
  );
  ...
  const rows = payable.map((o) => ({
    ...
    b2b_cost: Number(o.b2b_cost) || 0,
    partner_profit: Number(o.partner_profit) || 0,
  }));
  ...
  const sheetsHtml = rowPages
      ...
      <div class="card">
        ...
        ${
          Number(statement.totals.withdrawnPaid) > 0
            ? `<p class="mt">已加速提領 −${fmtNtd(statement.totals.withdrawnPaid)}</p>
        <p class="mt">本期應匯</p>
    ...
    <div class="totals">
      <div class="totals-box">
        ...
        <div><span>加速提領已匯（扣減）</span><span>−${fmtNtd(statement.totals.withdrawnPaid || 0)}</span></div>
    ...
    <div class="notes">
      ...
      <ul>
        ...
        <li>主流程為月結；夥伴另可申請加速提領。已匯款之加速提領會自本期應匯扣減，避免重複給付。</li>
      ...
      ${
        (statement.withdrawalsDeducted || []).length
          ? `<h4 class="mt">本期扣減之加速提領（${statement.counts.withdrawalsDeducted}）</h4><ul>${statement.withdrawalsDeducted
              .map(
                (w) =>
                  `<li>#${w.id} ${fmtNtd(w.amount)}${w.remittance_memo ? ` · ${escapeHtml(w.remittance_memo)}` : ""}</li>`,
              )
              .join("")}</ul>`
          : ""
      }
