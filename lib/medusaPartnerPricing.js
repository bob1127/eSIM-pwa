 ...
 * 2. Medusa variant.metadata.b2b_price／cost_price（上架／匯入時寫入的真實成本）
...
import { getCachedB2BMultiplier } from "./platformSettingsCache";
...
export function getPartnerB2BMarkupMultiplier() {
  // 優先使用 boss 後台設定（DB → 行程內快取）。快取由伺服器端 async 進入點
  // （fetchStoreListings／computeAuthoritativeStoreOrder／顯示 API）在請求開始時
  // 以 loadB2BMarkupMultiplier() 暖入。快取未命中時退回環境變數 → 預設值。
...
export function resolveApiWholesalePriceDetailed(
  ...
  // 匯入／同步常寫 cost_price（TWD），與 b2b_price 同義
  if (meta.cost_price != null && meta.cost_price !== "") {
    const cost = Number(meta.cost_price);
    if (Number.isFinite(cost) && cost > 0) {
      return { value: Math.round(cost), source: "metadata" };
    }
  }
