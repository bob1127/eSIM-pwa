...
import { enableLineTrafficAlertForMemberEmail } from "./lineTrafficAlert";
import { mergeWelcomeCouponToMember } from "./memberWelcomeBenefit";
...
export async function performLineAccountBind(
  ...
  if (member.source === "line") {
    ...
    const trafficAlert = await tryEnableTrafficAlert(
      supabaseAdmin,
      lineUserId,
      member,
    );
    try {
      await mergeWelcomeCouponToMember(supabaseAdmin, {
        email: member.email,
        lineUserId,
        userId: member.userId || null,
      });
    } catch (e) {
      console.warn("[lineAccountBind] 合併 welcome 券略過:", e.message);
    }
...
async function tryEnableTrafficAlert(supabaseAdmin, lineUserId, member) {
  try {
    return await enableLineTrafficAlertForMemberEmail(
      supabaseAdmin,
      lineUserId,
      member?.email,
    );
  } catch (e) {
    console.warn("[lineAccountBind] 開啟流量提醒失敗（不影響綁定）:", e.message);
