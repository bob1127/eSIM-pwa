 ...
 * - 旅客結帳流程會讀取 Supabase 的 referral_medusa_code 做映射；
 *   實際折扣趴數改由商品電信商 metadata 在套碼前同步進 Medusa（見
 *   pages/api/checkout/promotion.js）。
 */
...
export async function reconcilePartnerDiscountPromotion(token, partner) {
  ...
  // 實際趴數以商品 metadata 為準；此處只確保碼存在／啟用，用佔位趴數即可
  const percent = Number(partner.referral_discount_percent) || 10;
