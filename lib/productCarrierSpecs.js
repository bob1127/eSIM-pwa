...
import {
  formatHighSpeedQuotaLabel,
  getVariationOptionAttrs,
  injectHighSpeedQuotaIntoSpeedRule,
} from "./dataAmountLabel";
...
export function extractPreThrottleSpeedLabel(variation, specs = null) {
  ...
  const dataAmount =
    attrs.data_amount ||
    meta.data_amount ||
    getVariationOptionAttrs(variation).data_amount;
  const throttleBlob = [
    speedRule,
    specs?.speed_rule,
    variation?.rule_desc,
  ...
  if (
    dataAmount &&
    /總量|用完後降速|high[-\s]?speed.*afterward|Daily\s+\d/i.test(
      throttleBlob,
    ) &&
    !/每日\s*\d|Daily\s+\d/i.test(throttleBlob)
  ) {
    const quotaLabel = formatHighSpeedQuotaLabel(dataAmount);
...
export function resolveCarrierSpecs(product, carrierName, variation) {
  ...
  const matched = fromMeta
    ? findCarrierSpecEntry(fromMeta, carrierName) ||
      fromMeta.default ||
      soleCarrierFallback(fromMeta, carrierName) ||
      null
    : null;
  ...
  const dataAmount =
    attrs.data_amount ||
    variation?.metadata?.data_amount ||
    getVariationOptionAttrs(variation).data_amount;
  if (dataAmount && normalized.speed_rule) {
    normalized.speed_rule = injectHighSpeedQuotaIntoSpeedRule(
      normalized.speed_rule,
      dataAmount,
    );
  }
