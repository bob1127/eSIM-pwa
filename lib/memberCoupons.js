...
export async function runMemberLotterySpin(supabaseAdmin, member) {
  ...
  if (!LOTTERY_TEST_UNLIMITED) {
    const { data: existing, error: existErr } = await supabaseAdmin
      ...
      .select("id, prize_id, amount, coupon_id, play_day, created_at")
      .eq("email", email)
...
/** 查詢會員是否已抽過（終身） */
export async function getMemberLotteryPlayStatus(supabaseAdmin, email) {
  ...
  const { data, error } = await supabaseAdmin
    .from("member_lottery_plays")
    .select("id, prize_id, amount, play_day, created_at")
    .eq("email", normalized)
    .order("created_at", { ascending: true })
    .limit(1)
    .maybeSingle();
