...
import {
  formatMb,
  formatExpiryTaiwan,
  parseServiceDaysFromPlanSku,
  resolveEsimExpiryDisplay,
} from "./esimUsageFormat";
...
import {
  ...
  resolveEsimInstallState,
} from "./esimInstallStatus";
...
export async function queryEsimUsage({ iccid, topupId, endpoint } = {}) {
  ...
  try {
    ...
    const raw = {
      ...
      serviceDays:
        firstScalar(r.day, r.days, r.service_days) ||
        parseServiceDaysFromPlanSku(planName),
      validityPeriod: firstScalar(r.validity_period) || null,
...
export function formatUsageForLine(data) {
  ...
  if (resolveEsimInstallState(data) === "not_installed") {
    return [
      "📱 eSIM 用量查詢",
      "",
      data.productName ? `方案：${data.productName}` : null,
      "",
      `⚠️ ${ESIM_NOT_INSTALLED_USAGE_MESSAGE}`,
      "",
      `🌐 安裝教學與查詢：${getPublicSiteUrl()}/data-query`,
    ]
      .filter(Boolean)
      .join("\n");
  }
  ...
  const expiry = resolveEsimExpiryDisplay(data);
  const serviceDays = String(
    data.serviceDays ||
      parseServiceDaysFromPlanSku(data.productName || data.sku || "") ||
      "",
  ).trim();
  const lines = [
    "📊 eSIM 用量查詢結果",
    ...
    data.activatedAt
      ? `裝置啟用：${formatExpiryTaiwan(data.activatedAt) || String(data.activatedAt).slice(0, 16)}`
      : data.provisionedAt || data.createdAt
        ? `供應商開通：${formatExpiryTaiwan(data.provisionedAt || data.createdAt) || String(data.provisionedAt || data.createdAt).slice(0, 16)}`
        : null,
    expiry.line || null,
    expiry.kind === "plan_end" && serviceDays
      ? `※ ${serviceDays}天方案以日曆天計算（含啟用當日）`
      : null,
    "",
    `※ 用量非即時：供應商同步通常有約 30～60 分鐘延遲；手機顯示用量可能略高於此數字`,
  ].filter(Boolean);
