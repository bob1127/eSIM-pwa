...
export const TELECOM_TO_ALIAS = {
  "SK電信（韓國IP）": "sk-native",
  "LG U+ / SK電信": "lg-sk",
  "LG U+ / SK電信 5G 雙切換": "lg-skt-dual",
  "LG U+ / SKT 5G 雙切換": "lg-skt-dual", // 舊名
  "SK電信 5G": "skt",
  "SKT 5G": "skt", // 舊名
  "AU(KDDI) 真。吃到飽不降速": "au-kddi",
  "AU(KDDI) 高速數據": "au-kddi",
  "AU(KDDI)": "au-kddi",
  "AU(KDDI) 10Mbps": "au-kddi-10m",
  "AU KDDI 10Mbps": "au-kddi-10m",
  KDDI: "kddi",
  "IIJ Docomo（注意：需手動設定 APN）": "iij-docomo",
  "IIJ Docomo": "iij-docomo",
  "IIJ(DOCOMO)": "iij-docomo",
  "SoftBank（注意：Android 通常需手動 APN）": "softbank",
  SoftBank: "softbank",
  "SoftBank / KDDI": "softbank-kddi",
  "SoftBank / KDDI 10Mbps": "softbank-kddi-10m",
  "KDDI / SoftBank": "kddi-softbank",
  "KDDI / SoftBank / Docomo +": "kddi-sb-docomo",
  "KDDI / SoftBank / Docomo": "kddi-sb-docomo",
  中國移動: "cmcc",
  "CMCC+": "cmcc-plus",
  "CMCC 70Mbps": "cmcc-70",
  "CUCC+": "cucc-plus",
  // 短別名 cucc：吃到飽頁只有 CUCC+ 時可選中（與中文「中國聯通」並列）
  中國聯通: "cucc",
  "中國聯通 GPT + TikTok (CUCC)": "cucc-gpt",
  GPT: "gpt-tiktok",
  "GPT + TikTok": "gpt-tiktok",
  "Truemove H 當地號碼": "truemove",
  "Mobifone 當地號碼": "mobifone",
  "True 電信": "true-th-unlimited",
  "TRRE 電信": "true-th-unlimited", // 舊名
  "True Dtac": "true-th-unlimited", // 舊名
  ...
  // 總量型商品與每日型共用同一電信顯示名，別名可相同（依商品內選項還原）
  AIS: "ais",
  TRUE: "true",
  DTAC: "dtac",
  "DTAC / REAL FUTURE": "dtac-rf",
  Viettel: "viettel",
  Vinaphone: "vinaphone",
  Wintel: "wintel",
  "CSL / China Telecom HK": "csl-ct",
  "CUCC / China Telecom + CSL + CTM": "cucc-csl-ctm",
  "3HK": "3hk",
  // 舊名（已改為 3HK）— 別名仍解析到 3HK，避免舊連結失效
  "CSL / SmarTone": "3hk",
  "CSL / SmarTone（總量型）": "3hk",
  "CSL / SmarTone（每日型）": "3hk",
  "UMobile 5G": "umobile",
  "UMobile 5G 當地": "umobile",
  "Maxis / Celcom / Digi": "maxis-celcom-digi",
  "VIVO BRAZIL": "vivo-brazil",
  "Vivo Brazil": "vivo-brazil",
  "Verizon USA / AT&T USA": "verizon-att-us",
  "長天數 Verizon USA / AT&T USA": "long-verizon-att-us",
  "長天數 Verizon": "long-verizon",
  "US,CA 多網 A0": "usca-a0",
  "Verizon + Bell / Telus": "verizon-bell-telus",
  "T-Mobile / Verizon / AT&T + 加拿大": "tm-vz-att-ca",
  "Verizon / AT&T / T-Mobile + 加拿大多網": "vz-att-tm-ca-multi",
  "Bell/Telus + Verizon（A0）": "bell-telus-verizon-a0",
  "US&Canada Total B（T-Mobile/AT&T/Verizon + 加拿大多網）": "usca-total-b",
  "AT&T 美國號碼": "att-us-number",
  "Rogers + Movistar + Verizon USA / AT&T USA": "rogers-movistar-vz-att",
  // 澳洲學生／出差長天數：方案類型＋電信合併標籤
  "吃到飽 不限流量 (OPTUS)": "optus-unlim",
  "每日型 (OPTUS)": "optus-daily",
  "總量型 (OPTUS)": "optus-total",
  OPTUS: "optus",
  "Rogers + Movistar + Verizon USA / AT&T USA（A1）": "rogers-movistar-vz-att-a1",
  "WIND / Bell / TELUS +": "wind-bell-telus",
  "TELUS / BELL": "telus-bell",
  "Rogers / Bell / TELUS +": "rogers-bell-telus",
  "TELUS 原生": "telus-native",
};
...
function pickFromAvailable(candidate, available = []) {
  ...
  const want10 = /10mbps|10m/.test(want);
  const scored = available
    .map((a) => {
      const n = normalizeTelecomKey(a);
      const a10 = /10mbps|10m/.test(n);
      if (want10 !== a10 && (want10 || a10)) return null; // 速度檔次不同不互配
      if (!(n.includes(want) || want.includes(n))) return null;
      return { a, len: n.length };
    })
    .filter(Boolean)
    .sort((x, y) => y.len - x.len);
  if (scored[0]?.a) return scored[0].a;
  // 商品只有一個電信時（例如拆分後的 10Mbps 專屬頁），舊連結 au-kddi 應落到該電信
...
export function resolveTelecomQuery(raw, availableCarriers = []) {
  ...
  if (
    lower === "csl-smartone" ||
    lower === "csl-smartone-daily" ||
    lower === "csl-smartone-total"
  ) {
    return pickFromAvailable("3HK", availableCarriers);
  }

  // A) 手動別名表
  if (ALIAS_TO_TELECOM[lower]) {
    const mapped = pickFromAvailable(ALIAS_TO_TELECOM[lower], availableCarriers);
    if (mapped && (!availableCarriers.length || availableCarriers.includes(mapped))) {
      return mapped;
    }
    // cucc → 中國聯通：吃到飽頁選項名為 CUCC+ 時改選 CUCC+
    if (lower === "cucc" || lower === "cucc-plus") {
      const cuccHit = availableCarriers.find((a) =>
        /^cucc\+?$/i.test(String(a).trim()) || /聯通|cucc/i.test(a),
      );
      if (cuccHit) return cuccHit;
    }
...
/** 寫入 URL：無限流量／吃到飽 → unlimited；其餘保留如 5GB */
export function toDataAmountQueryValue(label) {
  const s = String(label || "").trim();
  if (!s) return "";
  if (/無限|unlimited|吃到飽/i.test(s)) return "unlimited";
...
export function resolveDataAmountQuery(raw, availableAmounts = []) {
  const s = decodeOnce(raw);
  if (!s) return "";
  if (/^unlimited$/i.test(s) || /無限|吃到飽/i.test(s)) {
    const hit = availableAmounts.find((a) =>
      /無限|unlimited|吃到飽/i.test(a),
    );
    return hit || "無限流量";
  }
