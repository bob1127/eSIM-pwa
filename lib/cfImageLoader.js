...
const WIDTH_BUCKETS = [360, 640, 960, 1280, 1600, 2048];
...
/**
 * 遠端原圖要過 CF Transformations，Dashboard「Allowed origins」必須先加該 host，
 * 否則會 403 + cf-resized err=9401（部落格 WP／Photon 就是這類）。
 * 用逗號分隔，例如：pub-xxx.r2.dev,images.jeko-esim.com.tw
 */
export function getCfRemoteOriginAllowlist() {
  return String(process.env.NEXT_PUBLIC_CF_IMAGES_REMOTE_ORIGINS || "")
    .split(",")
    .map((s) => s.trim().toLowerCase())
    .filter(Boolean);
}

export function isCfRemoteOriginAllowed(hostname) {
  const host = String(hostname || "")
    .trim()
    .toLowerCase();
  if (!host) return false;
  return getCfRemoteOriginAllowlist().includes(host);
}
...
export function shouldSkipCfImage(src) {
  ...
  if (src.includes("/product-gallery/") || src.includes("/sim/產品/") || /\/images\/[^/]*esim\.png/i.test(src)) {
    return true;
  }
  // 商城實體商品 Medusa /static 原圖：勿再經 CF 壓縮變糊
  if (/\/static\//i.test(src)) return true;
...
export default function cfImageLoader({ src, width }) {
  ...
  // 絕對網址：同源 → 相對 /cdn-cgi；外域僅在 allowlist 內才包（否則直接用原圖）
  const zone = getZoneOrigin();
  try {
    const srcUrl = new URL(src);
    const zoneHost = zone ? new URL(zone).host.toLowerCase() : "";
    const srcHost = srcUrl.host.toLowerCase();
    const sameHost = zoneHost && srcHost === zoneHost;
    if (sameHost) {
      return `/cdn-cgi/image/${opts}${encodeSourcePath(srcUrl.pathname)}${srcUrl.search}`;
    }
    if (!isCfRemoteOriginAllowed(srcHost)) {
      // WP Bluehost / i0.wp.com 等：原圖可開，硬包 CF 會 9401 整頁破圖
      return src;
    }
  } catch {
    return src;
