...
export function defaultHomepageCms(store = {}) {
  ...
  return {
    ...
    plans: {
      title: "Must-Have eSIM Selections",
      subtitle: "本賣場精選方案",
      shop_link_label: "查看 Jeko Shop 全部商品 →",
      shop_link_href: "/shop/",
      /** 國家分類 Tab 自訂順序（key 陣列） */
      category_order: [],
    },
  };
}
...
function normalizeCategoryOrder(raw) {
...
export function mergeHomepageCms(store, raw) {
  ...
  const discIn =
    incoming.discover && typeof incoming.discover === "object"
      ...
      : {};
  const plansIn =
    incoming.plans && typeof incoming.plans === "object" ? incoming.plans : {};
  ...
  return {
    ...
    plans: {
      title:
        clip(plansIn.title ?? base.plans.title, MAX_TEXT) || base.plans.title,
      subtitle:
        clip(plansIn.subtitle ?? base.plans.subtitle, MAX_SUB) ||
        base.plans.subtitle,
      shop_link_label:
        clip(plansIn.shop_link_label ?? base.plans.shop_link_label, MAX_TEXT) ||
        base.plans.shop_link_label,
      shop_link_href: safeHref(
        plansIn.shop_link_href ?? base.plans.shop_link_href,
        base.plans.shop_link_href,
      ),
      category_order: normalizeCategoryOrder(
        plansIn.category_order ?? base.plans.category_order,
      ),
    },
  };
}
