...
export const STATIC_PAGE_ENTRIES = [
  ...
  },
  {
    ...
    title: "任務牆",
    href: "/missions",
    keywords: "任務牆 體驗 互惠 分潤 line 接任務",
    excerpt: "加入官方 LINE，申請體驗、互惠、有酬與分潤任務。",
  },
  {
    id: "page-wizard",
    title: "智慧選品精靈",
  ...
  },
  {
    ...
    title: "蝦皮快速兌換",
    href: "/shopee-qrcode",
    keywords: "蝦皮 兌換 qrcode 訂單",
    excerpt: "輸入蝦皮訂單編號兌換 eSIM QR Code。",
  },
  {
    id: "page-check-esim",
    title: "檢查手機是否支援 eSIM",
...
export const LEGAL_ENTRIES = [
  ...
  },
  {
    ...
    title: "付款、發票與退款",
    href: "/terms",
    keywords: "付款 發票 電子發票 退款 line pay 信用卡",
    excerpt: "支援開立電子發票；付款方式與退款相關說明。",
  },
  {
    id: "qa-einvoice",
    source: "qa",
    title: "有支援開立電子發票嗎？",
    href: "/qa",
    keywords: "電子發票 統編 發票 FAQ",
    excerpt: "Jeko eSIM 支援開立電子發票，付款後依 Email／會員資料開立。",
  },
  {
    id: "terms-partner",
    source: "terms",
    title: "合作夥伴與分潤",
    href: "/terms",
    keywords: "夥伴 分潤 專屬連結 專屬商店 加價 文章 免責",
    excerpt: "專屬連結／專屬商店合作、分潤規則，以及夥伴自訂文章內容免責。",
  },
  {
    id: "terms-content-disclaimer",
    source: "terms",
    title: "自訂文章與上傳內容免責",
    href: "/terms",
    keywords: "免責 版權 著作權 非法 侵權 夥伴原創 文章 圖片",
    excerpt:
      "夥伴發布文章或使用無版權資產、從事非法用途時，責任由該夥伴承擔，平台不負責。",
  },
  {
    id: "terms-liability",
    source: "terms",
...
export function stripHtml(html) {
  return stripWpReadMore(String(html || ""))
    .replace(/<[^>]*>/g, " ")
    .replace(/&\w+;/g, " ")
    .replace(/\s+/g, " ")
    .trim();
...
/** 商品目錄即時搜尋（前台／API 共用） */
export function searchProductSummaries(products, query, limit = 8) {
  const q = String(query || "").trim();
  if (!q) return [];
...
/** 靜態頁 + 商品（不含 WordPress 文章） */
export function searchInstantSite(query, products, { limit = 20 } = {}) {
  const q = String(query || "").trim();
  if (!q) return [];
  const productHits = searchProductSummaries(products, q, 8);
  const staticHits = searchStaticEntries(q, { limit: 10 });
  return [...productHits, ...staticHits]
    .sort(compareSearchResults)
    .slice(0, limit);
}
