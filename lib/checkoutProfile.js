...
/** 結帳可顯示／可寄送的 email（排除 LINE 虛擬信箱） */
function pickCheckoutEmail(...candidates) {
  for (const c of candidates) {
    const s = String(c ?? "").trim();
    if (s && !isLineSyntheticEmail(s)) return s;
  }
  return "";
}
...
export function loadSavedCheckoutProfile() {
  ...
  try {
    ...
    return {
      ...
      email: pickCheckoutEmail(data.email),
...
export function saveCheckoutProfile(formData) {
  ...
  try {
    const payload = {
      ...
      email: pickCheckoutEmail(formData.email),
