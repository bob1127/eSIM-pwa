 ...
 * 點擊時以 max-height 過渡展開收合（不會有版面跳動）。
 *
...
export function faqAccordion(items, { defaultOpenIndex = 0 } = {}) {
  const rows = items
    .map(({ question, answerHtml }, i) => {
      ...
      return `
    <div class="jeko-faq-item${isOpen ? " is-open" : ""}" data-jeko-faq-item style="${i > 0 ? "border-top:1px solid #eef2f7;" : ""}">
      <h3
        ...
        style="margin:0;padding:18px 6px;display:flex;align-items:center;justify-content:space-between;gap:16px;font-size:15px;font-weight:600;color:#1e293b;"
      >
        <span style="flex:1;">${question}</span>
        <span
          class="jeko-faq-icon material-symbols-outlined"
          aria-hidden="true"
          style="font-size:22px;color:#2D5BE3;flex-shrink:0;transition:transform .3s ease;transform:rotate(${isOpen ? "180" : "0"}deg);"
          >expand_more</span
        >
      </h3>
      <div
        ...
        id="${panelId}"
        style="overflow:hidden;max-height:${isOpen ? "999px" : "0px"};transition:max-height .3s ease;"
      >
        <div style="padding:0 6px 20px;font-size:14px;color:#64748b;line-height:1.8;">${answerHtml}</div>
      </div>
    </div>`;
    })
    .join("");
  ...
  return `
  <div style="background:#FFFFFF;border:1px solid #e2e8f0;border-radius:16px;padding:6px 18px;">
    ${rows}
  </div>`;
}
