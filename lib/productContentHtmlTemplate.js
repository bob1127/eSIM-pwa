...
export function usageAdvantagesSection({
  ...
  const cards = (items || [])
    ...
    <div class="jeko-adv-card" style="display:flex;align-items:flex-start;gap:16px;background:#FFFFFF;border:1px solid #E5E7EB;border-radius:16px;padding:24px 20px;box-shadow:0 4px 14px rgba(15,23,42,0.05);min-width:0;">
      <div class="jeko-adv-icon" aria-hidden="true" style="width:48px;height:48px;border-radius:12px;background:#EEF3FF;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
        <span class="material-symbols-outlined" style="font-size:28px;color:#2D5BE3;">${iconName}</span>
      </div>
      <div class="jeko-adv-body" style="min-width:0;">
        <h4 class="jeko-adv-title" style="margin:0 0 6px;font-size:17px;font-weight:700;color:#2D5BE3;line-height:1.35;">${cardTitle}</h4>
        <p class="jeko-adv-desc" style="margin:0;font-size:14px;color:#334155;line-height:1.65;">${descHtml}</p>
      </div>
    </div>`,
    )
    .join("");
  ...
  <div class="jeko-usage jeko-adv">
    <div class="jeko-section-head jeko-adv-head" style="text-align:center;margin-bottom:28px;">
      <h3 style="margin:0;font-size:24px;font-weight:800;color:#1e293b;letter-spacing:-0.01em;">${title}</h3>
      ${subtitle ? `<p style="margin:8px auto 0;max-width:36rem;font-size:15px;color:#64748b;">${subtitle}</p>` : ""}
    </div>
    <div class="jeko-adv-grid" style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:20px;">
      ${cards}
    </div>
  </div>`;
}
