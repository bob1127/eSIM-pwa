...
export async function verifyLineIdToken(idToken) {
  ...
  try {
    ...
    return { ok: false, error: `verify_request_failed:${e.message}` };
  }
  ...
  // LINE verify 端點已檢查 aud；此處為縱深防禦，避免拿別的 Channel 簽發的合法
  // token 誤套用到本站（例如共用同一組 server 但服務多個 LINE App 時）。
  if (data.aud && String(data.aud) !== clientId) {
