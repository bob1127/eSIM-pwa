...
export async function buildWelcomeFollowMessage(welcomeState = {}) {
  ...
  const isReFollow = Boolean(welcomeState?.isReFollow);
  const alreadyRedeemed = Boolean(welcomeState?.alreadyRedeemed);
  const alreadyClaimed = Boolean(welcomeState?.alreadyClaimed);

  const siteRoot =
    String(siteUrl || "").replace(/\/$/, "") || "https://www.jeko-esim.com.tw";

  const text = buildWelcomeFollowTextFromSettings(settings, {
    siteUrl: siteRoot,
    code,
    isReFollow,
    alreadyRedeemed,
  ...
  return [
    ...
    buildIccidUsageFlexMessage(settings),
  ];
}
