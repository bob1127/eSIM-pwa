...
import { fetchMedusaProductById, fetchMedusaThumbnailMapByIds } from "./medusaStoreApi";
...
export async function fetchStoreProductsForStorefront(store) {
  ...
  let liveThumbnailMap = {};
  try {
    liveThumbnailMap = await fetchMedusaThumbnailMapByIds(medusaIds);
  } catch (err) {
    console.warn(
      "[fetchStoreProductsForStorefront] thumbnail refresh failed:",
      err?.message || err,
    );
  }
  ...
  for (const sp of listings) {
    ...
    if (p) {
      ...
      const medusaKey = medusaId ? String(medusaId) : "";
      const image =
        (medusaKey && liveThumbnailMap[medusaKey]) ||
        resolveMedusaImageUrl(p.image_url) ||
        null;
      out.push({
