 ...
 * - 匯率改用固定保守值（可用環境變數調整），不再打第三方即時匯率 API——
...
// 固定保守匯率（抓寬鬆一點：低估底價比高估底價危險，寧可保守），
// 可用環境變數覆寫，不再依賴第三方即時匯率 API。
const FIXED_FX = {
  USD: Number(process.env.ESIM_FX_RATE_USD) || 33.0,
  HKD: Number(process.env.ESIM_FX_RATE_HKD) || 4.5,
};
...
export async function resolveLivePlanCostTWD(sku) {
  ...
  try {
    ...
    if (!entry) {
      const mappedId = PLAN_ID_MAP[key];
