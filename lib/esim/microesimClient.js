 ...
 * - 會嘗試去掉 #tag、本地後綴，並同時查「原字／逗號改 -」的 planMap key
 * - 測試環境可強制改寫為 ESIM_TEST_PLAN_ID
 * 注意：此函式不做「幽靈舊名偷換貨」防護；發貨請改走
 * lib/esim/planAvailability.validatePlanAvailability，只用回傳的 channelDataplanId。
 */
export function resolveChannelDataplanId(rawPlanId, planIdMap = {}) {
  if (shouldForceTestPlan()) {
    return ESIM_TEST_PLAN_ID;
  }
  const raw = String(rawPlanId || "")
    .trim()
    .replace(/\u200B/g, "");
  if (!raw) return "";

  // 已是 UUID／32 hex → 直接用（不做 map）
  if (
  ...
  for (const base of [raw, noHash]) {
    for (const suf of [
      ...
      if (base.endsWith(suf)) candidates.add(base.slice(0, -suf.length));
    }
    if (base.includes(",")) candidates.add(base.replace(/,/g, "-"));
  }

  for (const key of candidates) {
    if (planIdMap[key]) return String(planIdMap[key]);
  }
  // 查無 map 時回傳原字串（可能是供應商 id）；呼叫端應再以目錄核對
  return raw;
}
