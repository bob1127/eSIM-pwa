...
import { fetchWpPosts, fetchWpPostBySlug, resolveWpBannerImage } from "@/lib/wordpress";
...
import { parseCreatorKey, parsePostKey, postKeyFromPost } from "@/lib/blogCreator";
...
/**
 * 依 blog_post_likes 列還原文章卡片（與 CreatorFollowFeed 卡面欄位相容）
 * @param {{ post_key: string, created_at?: string }[]} likeRows
 */
export async function loadSavedPostsFromLikes(likeRows = []) {
  const rows = Array.isArray(likeRows) ? likeRows : [];
  const out = [];
  for (const row of rows) {
    const parsed = parsePostKey(row?.post_key);
    if (!parsed) continue;
    try {
      if (parsed.type === "jeko") {
        const wp = await fetchWpPostBySlug(parsed.slug);
        if (!wp) continue;
        const [post] = await attachPostStats([mapWpPost(wp)]);
        if (!post?.href) continue;
        out.push({
          ...post,
          creatorKey: "jeko",
          creatorName: "Jeko eSIM",
          creatorAvatar: "/images/Logo/icon-192.png",
      ...
      const store = await fetchActiveStoreByDomain(parsed.domain);
      if (!store) continue;
      const raw = await fetchPartnerBlogPostBySlug(parsed.slug, store);
      if (!raw) continue;
      const [post] = await attachPostStats(
        [mapPartnerPost(raw, parsed.domain)],
        { partnerDomain: parsed.domain },
      );
      if (!post?.href) continue;
      out.push({
        ...post,
        creatorKey: parsed.creatorKey,
        creatorName: store.store_name || parsed.domain,
        creatorAvatar: store.logo_url || "/images/Logo/icon-192.png",
        savedAt: row.created_at || null,
      });
    } catch {
      /* skip missing posts */
