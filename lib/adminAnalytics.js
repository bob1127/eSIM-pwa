...
/** @deprecated 使用 orderWithinDays */
export function orderWithinDays(createdAt, days) {
  return withinDays(createdAt, days);
}

/** 折線圖天數；「全部」時用近 30 日趨勢，避免一次畫上千個點 */
function chartDaysFromFilter(days) {
  const n = Number(days);
  if (!n || n <= 0 || n >= 9999) return 30;
  return n;
}
...
export function buildPartnerSalesReport(orders = [], filters = {}) {
  ...
  const analytics = buildAdminAnalytics(
    filtered,
    chartDaysFromFilter(filters.days),
  );
  ...
  const partnerAllTimeCounts = {};
  orders.forEach((o) => {
    const pid = o.partner_id || 0;
    if (!pid) return;
    partnerAllTimeCounts[pid] = (partnerAllTimeCounts[pid] || 0) + 1;
  });
  ...
  orders.forEach((o) => {
    ...
    if (!sid) return;
    storeAllTimeCounts[sid] = (storeAllTimeCounts[sid] || 0) + 1;
  });
  ...
  for (const row of Object.values(partnerMap)) {
    row.ordersAllTime = partnerAllTimeCounts[row.partnerId] ?? row.orders;
  }
  ...
  for (const row of Object.values(storeMap)) {
    row.ordersAllTime = storeAllTimeCounts[row.storeId] ?? row.orders;
  }
  ...
  return {
    ...
    orders: filtered.map((o) => ({
      ...
      medusaOrderId: o.medusa_order_id || "",
