...
import { hydrateEsimProfileFields } from "./esimInstallLinks.js";
...
function setupBlock(profile) {
  ...
  const facts = [
    ...validityLines.map((line) => esc(line)),
  ...
  if (!validityLines.length) {
    facts.unshift(
      days
        ? `使用天數：${esc(days)} 天（自啟動日起算）`
        : "使用天數：—",
      validityPeriod
        ? `未啟用有效期：${esc(validityPeriod)} 天`
        : "未啟用有效期：—",
    );
  }
...
function profileCard(rawProfile, idx, total, origin = "", orderCtx = {}) {
  const profile = hydrateEsimProfileFields(rawProfile);
  const androidCode =
