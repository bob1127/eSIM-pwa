...
const DEFAULT_LOGO = `${SITE_URL}/images/Logo/icon-512.png`;
...
function imageList(product) {
  ...
  return unique.length ? unique : [absoluteUrl("/images/Logo/icon-512.png")];
}
...
function resolveAggregateRating(reviewAggregate) {
  const r = reviewAggregate?.reviewCount > 0 ? reviewAggregate : PRODUCT_AGGREGATE_RATING;
  return {
    ratingValue: Number(r.ratingValue) || PRODUCT_AGGREGATE_RATING.ratingValue,
    reviewCount: Number(r.reviewCount) || PRODUCT_AGGREGATE_RATING.reviewCount,
    ratingCount:
      Number(r.ratingCount || r.reviewCount) ||
      PRODUCT_AGGREGATE_RATING.ratingCount,
    bestRating: Number(r.bestRating) || 5,
    worstRating: Number(r.worstRating) || 1,
  };
}

function aggregateRatingNode(reviewAggregate) {
  const r = resolveAggregateRating(reviewAggregate);
  return {
...
/** Google 複合式結果用的個別 Review（需與頁面可見內容一致） */
function reviewNodes(canonical, reviewAggregate) {
  const list = Array.isArray(reviewAggregate?.reviews)
...
function offerNode({ url, price, sku, name }) {
  const amount = toTwdAmount(price);
  if (amount == null) return undefined;
  return {
    "@type": "Offer",
    "@id": sku ? `${url}#offer-${sku}` : `${url}#offer`,
    name: name || undefined,
    url,
    price: String(amount),
    priceCurrency: "TWD",
    ...
    eligibleTransactionVolume: {
      ...
      minPrice: String(amount),
      priceCurrency: "TWD",
    },
    inventoryLevel: {
      "@type": "QuantitativeValue",
      value: 999,
      unitText: "digital units",
    },
...
export function buildProductJsonLdNodes({
  ...
  const reviews = reviewNodes(canonical, reviewAggregate);
  ...
  const productNode = {
    ...
    alternateName: [
      product?.subtitle,
      dest.nameZh ? `${dest.nameZh} eSIM` : null,
      `${SITE_NAME} ${name}`,
    ].filter(Boolean),
    description,
    image: imageObjs,
    ...
    additionalProperty: [
      ...additionalProperties(variation, dest.nameZh),
      {
        "@type": "PropertyValue",
        name: "供應狀態",
        value: "銷售中／可供貨（InStock）",
      },
      {
        "@type": "PropertyValue",
        name: "配送方式",
        value: "數位交付（Email／會員中心 QR Code）",
      },
    ],
    aggregateRating: aggregateRatingNode(reviewAggregate),
    review: reviews.length ? reviews : undefined,
    offers,
    potentialAction: {
      "@type": "BuyAction",
      target: absoluteUrl("/Cart"),
      name: "加入購物車並結帳",
    },
  ...
  const items = (products || []).slice(0, 30).map((p, i) => {
    ...
    return {
      ...
      item: compact({
        ...
        aggregateRating: aggregateRatingNode(null),
        offers: price
          ? {
              "@type": "Offer",
              url,
