...
/** 正式站／Vercel：不可把圖留在訪客瀏覽器的 localhost */
function isProdImageRuntime() {
  return (
...
function rewriteLocalHostToBackend(url) {
  if (!url || !isLocalHostUrl(url)) return url;
  try {
    const u = new URL(url);
    return `${getBackendOrigin()}${u.pathname}${u.search}${u.hash}`;
  } catch {
    return url;
  }
}
...
import {
  LEGACY_ROOT_PRODUCT_IMAGE_REWRITES,
  isTransparentProductImageSrc,
} from "./countryProductImages";
...
export function shouldBypassImageOptimization(src) {
  ...
  if (isTransparentProductImageSrc(src)) return true;
...
export function resolveMedusaImageUrl(url) {
  ...
  let trimmed = rewriteLegacySiteImagePath(url.trim());
  if (!trimmed) return null;

  const backendOrigin = getBackendOrigin();
  const prod = isProdImageRuntime();
  const keepLocal = isLocalDevBackend() && !prod;

  // 正式站：DB 若仍是 localhost／127.0.0.1 → 改寫到 ASSET／BACKEND
  if (isLocalHostUrl(trimmed) && !keepLocal) {
    trimmed = rewriteLocalHostToBackend(trimmed);
  }
  ...
  if (trimmed.startsWith("/static/")) {
    ...
    return ensureHttpsAssetUrl(`${backendOrigin}${trimmed}`);
  }

  // 本機開發 + DB 仍是 localhost URL → 不要改寫到正式站
  if (keepLocal && isLocalHostUrl(trimmed)) {
    return trimmed;
  }
  ...
  return ensureHttpsAssetUrl(rewriteLegacySiteImagePath(resolved));
}
