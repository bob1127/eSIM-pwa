...
const coverage =
  "伊斯坦堡、安卡拉、伊茲密爾、安塔利亞、棉花堡、卡帕多奇亞等土耳其主要城市與觀光路線。";
const CARRIER = "AVEA TURKEY / VODAFONE TURKEY +";
...
export function turkeyCompareTableSection() {
  const table = dataTable(
    ["產品", "運營商", "最適合", "優點與注意事項"],
    [
      [
        `<strong>土耳其吃到飽</strong>`,
        "Avea／Vodafone<br>英國／波蘭 IP",
        "整天連線、觀光自駕",
        `${compareBullets([
          "每日約 1GB 高速後約 10Mbps 吃到飽。",
          "支援熱點與 ChatGPT／TikTok／Gemini。",
        ])}<div style="margin-top:8px;">${link("/product/turkey/turkey-unlimited-esim/", "查看吃到飽")}</div>`,
      ],
      [
        `<strong>土耳其每日／總量</strong>`,
        "Avea／Vodafone",
        "控管用量",
        `${compareBullets([
          "每日／總量高速用完後約 128kbps。",
        ])}<div style="margin-top:8px;">${link("/product/turkey/turkey-daily-esim/", "每日型")} · ${link("/product/turkey/turkey-total-esim/", "總量型")}</div>`,
      ],
    ],
  );
  return `<h4 style="margin:0 0 12px;font-size:16px;font-weight:700;color:#1e293b;">哪款土耳其 eSIM 最適合您？</h4>${table}`;
}
...
function otherActivate() {
  return otherInfoBlock([
    {
      title: "安裝提醒",
      html: `<span class="jeko-sum-warn">建議抵達土耳其覆蓋範圍後再安裝／啟用 eSIM。</span>提前安裝請關閉行動數據，避免效期提前開始。`,
      marginBottom: 12,
    },
...
function buildDetailed({ speedHtml, planTypeLabel, introParas, bullets }) {
  const planHtml = planCard(
    basePairs({
      carrierHtml: "Avea／Vodafone Turkey 4G／LTE／5G",
      speedHtml,
...
export const TR_UNLIM_DETAILED = buildDetailed({
  speedHtml: "4G／LTE／5G；每日約 1GB 高速後維持約 10Mbps 吃到飽",
  planTypeLabel: "吃到飽",
  introParas: [
    "jeko eSIM 土耳其吃到飽，走 <strong>AVEA／Vodafone＋</strong>，出網為<strong>英國／波蘭 IP</strong>。",
    "每日約 1GB 高速後約 10Mbps 無限流量。支援熱點與 ChatGPT／TikTok／Gemini。僅數據。",
  ],
  bullets: [
    "運營商：Avea／Vodafone Turkey・英國／波蘭 IP",
...
export const TR_DAILY_DETAILED = buildDetailed({
  speedHtml: "4G／LTE／5G；每日高速用完後約 128kbps（每日重置）",
  planTypeLabel: "每日型",
  introParas: [
    "jeko eSIM 土耳其每日型，走 <strong>AVEA／Vodafone＋</strong>，適合控管每日用量。",
...
export const TR_TOTAL_DETAILED = buildDetailed({
  speedHtml: "4G／LTE／5G；總量高速用完後約 128kbps 可持續使用",
  planTypeLabel: "總量型",
  introParas: [
    "jeko eSIM 土耳其總量型，走 <strong>AVEA／Vodafone＋</strong>，於有效天數內共用固定總流量。",
...
export const TR_USAGE = usageAdvantagesSection({
  title: "使用 jeko 土耳其 AVEA／Vodafone＋ eSIM 的優勢",
  subtitle: "雙網・觀光路線",
  items: [
    { iconName: "cell_tower", title: "Avea／Vodafone", descHtml: "伊斯坦堡與主要觀光城市覆蓋較完整。" },
    { iconName: "speed", title: "吃到飽或控管用量", descHtml: "可選每日 1GB 後約 10Mbps，或每日／總量方案。" },
    ...
    { iconName: "flight_land", title: "抵達再啟用", descHtml: "建議抵達土耳其後再安裝／開啟行動數據。" },
  ],
});
...
function sharedFaq(name) {
  return [
    ...
    },
    {
      ...
      answerHtml: p(
        `效期於 eSIM <strong>連接到支援的網路並開始產生數據流量</strong>後開始。建議抵達土耳其後再啟用。`,
        0,
      ),
    },
...
export const TR_FAQ_UNLIM = faqAccordion(
  [
    ...sharedFaq("土耳其吃到飽 eSIM"),
    {
      question: "每日 1GB 高速用完會怎樣？",
      answerHtml: p(`進入約 <strong>10Mbps</strong> 吃到飽（非斷網），隔日重置高速額度。`, 0),
    },
  ],
  { defaultOpenIndex: 0 },
);

export const TR_FAQ_DAILY = faqAccordion(
  [
    ...sharedFaq("土耳其每日型 eSIM"),
    {
      question: "每日額度用完會怎樣？",
      answerHtml: p(`高速用完後一般降速至約 <strong>128kbps</strong>，隔日重置。`, 0),
    },
  ],
  { defaultOpenIndex: 0 },
);

export const TR_FAQ_TOTAL = faqAccordion(
  [
    ...sharedFaq("土耳其總量型 eSIM"),
    {
      question: "總量用完會怎樣？",
      answerHtml: p(`高速用完後一般降速至約 <strong>128kbps</strong> 可持續使用。`, 0),
    },
  ],
  { defaultOpenIndex: 0 },
);

export { CARRIER as TR_TELECOM };
export default {
  TR_UNLIM_DETAILED,
  TR_DAILY_DETAILED,
  TR_TOTAL_DETAILED,
