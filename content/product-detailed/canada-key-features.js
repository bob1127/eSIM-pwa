...
const EXP_ROAM =
  "高速額度內：溫哥華、多倫多、蒙特婁都會區 4G／5G 測速常見可到數十 Mbps。高速用完後降速約 128kbps 可持續使用——傳訊／輕量網頁勉強可以，影音與熱點會明顯困難。出網為香港／新加坡 IP，ChatGPT／TikTok 不保證。僅供參考。";

const EXP_MULTI =
  "高速額度內：加拿大與美國都會區 4G／5G 測速常見可到數十 Mbps。總量用完後會斷網，無法繼續使用，請預留餘量。出網為波蘭 IP 漫遊節點；選品標示支援 ChatGPT、TikTok、Gemini。僅供參考。";

const EXP_NATIVE =
  "TELUS 當地線路、加拿大 IP。高速額度內都會區測速常見可到數十 Mbps；用完後約 128kbps 可持續使用。含本地通話／簡訊與部分國際通話分鐘。僅供參考。";

const EXP_DAILY_ROAM =
  "每日高速額度內：溫哥華、多倫多、蒙特婁都會區 4G／5G 測速常見可到數十 Mbps。額度用完後降速約 128kbps，隔日重置——傳訊／輕量網頁勉強可以，影音與熱點會明顯困難。出網為香港／新加坡 IP，ChatGPT／TikTok 不保證。僅供參考。";

const EXP_DAILY_MULTI =
  "每日高速額度內：加拿大與美國都會區 4G／5G 測速常見可到數十 Mbps。額度用完後降速約 512kbps，隔日重置；導航與傳訊通常仍可用。出網為波蘭 IP；選品標示支援 ChatGPT、TikTok、Gemini。僅供參考。";

const EXP_UNLIM =
  "美加主要城市 4G／5G 測速常見可到數十 Mbps，山區、邊境與室內會下降。吃到飽為 FUP，不會固定鎖死某一 Mbps，繁忙時段可能變慢。導航、傳訊通常沒問題。出網為波蘭 IP。僅供參考。";

export function canadaRoamTotalKeyFeatures() {
  return pack(
    [
      "本方案為 **TELUS／Bell** 加拿大總量型 eSIM（漫遊批發），覆蓋加拿大主要城市與交通沿線。",
...
export function canadaMultiTotalKeyFeatures() {
  return pack(
    [
      "本方案為 **美加多網** 總量型 eSIM（批發 SKU `USA & Canada-Total*`），單一 eSIM 可在加拿大與美國使用，**不含墨西哥**。",
...
export function canadaRoamDailyKeyFeatures() {
  return pack(
    [
      "本方案為 **TELUS／Bell** 加拿大每日型 eSIM（漫遊批發），覆蓋加拿大主要城市與交通沿線。",
...
export function canadaMultiDailyKeyFeatures() {
  return pack(
    [
      "本方案為 **美加多網** 每日型 eSIM（批發 SKU `USA & Canada-Daily*`），單一 eSIM 可在加拿大與美國使用，**不含墨西哥**。",
      "**為什麼選擇 Rogers／Bell／TELUS＋？**",
      "**加拿大 6 網**：Rogers、FIDO、Bell、Telus、Videotron、SaskTel（多數 5G；SaskTel 為 LTE）。",
      "**美國 3 網**：Verizon、AT&T、T-Mobile 4G／5G，跨境自駕／飛往美國免再換卡。",
...
export function canadaUnlimitedKeyFeatures() {
  return pack(
    [
      "本方案為 **美加吃到飽** eSIM（批發 SKU `US&Canada-unlimited*`），單一 eSIM 可在加拿大與美國使用，**不含墨西哥**。",
