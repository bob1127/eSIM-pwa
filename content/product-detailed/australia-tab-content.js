/**
 * 澳洲 eSIM — 下方 tab
 * Medusa：australia-unlimited/daily/total-esim｜OPTUS
 */
import {
...
const coverage =
  "雪梨、墨爾本、布里斯本、黃金海岸、珀斯等澳洲主要城市與熱門觀光／交通沿線。";
...
export function australiaCompareTableSection() {
  const table = dataTable(
    ...
    [
      [
        ...
        ])}<div style="margin-top:8px;">${link("/product/australia/australia-unlimited-esim/", "查看吃到飽")}</div>`,
      ],
      [
        `<strong>澳洲每日／總量</strong>`,
        "OPTUS",
        "控管用量",
        `${compareBullets([
          "每日／總量高速用完後約 128kbps。",
          "澳紐連遊可另選紐澳雙切換。",
        ])}<div style="margin-top:8px;">${link("/product/australia/australia-daily-esim/", "每日型")} · ${link("/product/australia/australia-total-esim/", "總量型")} · ${link("/product/anz/anz-unlimited-esim/", "紐澳")}</div>`,
      ],
    ],
  );
  return `<h4 style="margin:0 0 12px;font-size:16px;font-weight:700;color:#1e293b;">哪款澳洲 eSIM 最適合您？</h4>${table}`;
}
...
function otherActivate() {
  return otherInfoBlock([
    {
      title: "安裝提醒",
      html: `<span class="jeko-sum-warn">建議抵達澳洲覆蓋範圍後再安裝／啟用 eSIM。</span>提前安裝請關閉行動數據，避免效期提前開始。`,
      marginBottom: 12,
    },
...
function sharedFaq(productName) {
  return [
    ...
    },
    {
      ...
      answerHtml: p(
        `效期於 eSIM <strong>連接到支援的網路並開始產生數據流量</strong>後開始。建議抵達澳洲後再啟用。`,
        0,
      ),
    },
    {
...
export const AU_UNLIM_DETAILED = buildDetailed({
  carrierHtml: "Optus 4G／LTE／5G",
  speedHtml: "4G／LTE／5G；吃到飽不限流量（FUP，實際速度依環境與擁塞而定）",
  planTypeLabel: "吃到飽",
  routeHtml: "漫遊（新加坡 IP）",
  introParas: [
    "jeko eSIM 澳洲吃到飽方案，走 <strong>OPTUS</strong>，出網為<strong>新加坡 IP</strong>，適合雪梨、墨爾本等都會與打工度假行程。",
    "公平使用政策（FUP）下可持續上網。支援熱點與 ChatGPT／TikTok／Gemini。僅數據、無門號／傳統通話／簡訊。",
  ],
  bullets: [
    "運營商：Optus・新加坡 IP・4G／LTE／5G",
    "流量：吃到飽不限流量（FUP）",
...
export const AU_DAILY_DETAILED = buildDetailed({
  carrierHtml: "Optus 4G／LTE／5G",
  speedHtml:
  ...
  routeHtml: "漫遊（新加坡 IP）",
  introParas: [
    "jeko eSIM 澳洲每日型，走 <strong>OPTUS</strong>，適合想控管每日用量的旅客。",
...
export const AU_TOTAL_DETAILED = buildDetailed({
  carrierHtml: "Optus 4G／LTE／5G",
  speedHtml:
  ...
  routeHtml: "漫遊（新加坡 IP）",
  introParas: [
    "jeko eSIM 澳洲總量型，走 <strong>OPTUS</strong>，於有效天數內共用固定總流量。",
...
const usageCommon = [
  ...
  },
  {
    ...
    title: "抵達再啟用",
    descHtml: "建議抵達澳洲覆蓋範圍後再安裝／開啟行動數據。",
  },
];

export const AU_USAGE = buildUsage({
  title: "使用 jeko 澳洲 OPTUS eSIM 的優勢",
  subtitle: "三大電信之一・都會覆蓋",
  items: [
    {
      iconName: "cell_tower",
      title: "Optus 4G／5G",
      descHtml: "雪梨、墨爾本、布里斯本、黃金海岸、珀斯等熱門行程較穩。",
    },
    {
      iconName: "terrain",
      title: "偏遠請預留",
      descHtml: "內陸、沙漠與偏遠地區訊號會明顯下降。",
    },
    {
      iconName: "wifi_tethering",
      title: "支援熱點",
...
export const AU_FAQ_UNLIM = buildFaq([
  ...sharedFaq("澳洲吃到飽 eSIM"),
  {
    question: "吃到飽會限速嗎？",
    answerHtml: p(
...
export const AU_FAQ_DAILY = buildFaq([
  ...sharedFaq("澳洲每日型 eSIM"),
  {
    question: "每日額度用完會怎樣？",
    answerHtml: p(
...
export const AU_FAQ_TOTAL = buildFaq([
  ...sharedFaq("澳洲總量型 eSIM"),
  {
    question: "總量用完會怎樣？",
    answerHtml: p(
