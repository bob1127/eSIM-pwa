...
export function softbankKddiDailyKeyFeatures() {
  return pack(
    [
      ...
      "**安裝即用**：多數手機可自動帶入 APN，建議抵達日本後再安裝／啟用 eSIM。",
      "**支援熱點與常用 App**：熱點分享；選品標示支援 ChatGPT、TikTok、Gemini。",
      "**適合誰**：短途／多日行程、偏好雙網穩定、不想手動設 APN 的旅客（相對原生 SoftBank／IIJ）。",
    ],
    "高速額度內：東京／大阪等都會區測速常見可到數十 Mbps（視訊號與負載而定）。高速用完後降速至約 128kbps，測速約 0.1Mbps 等級——傳訊／輕量網頁勉強可以，影音與即時導航會明顯困難。新加坡 IP 下多數社群可正常使用，僅供參考。",
  );
}

/** SoftBank 原生：JapanSB-Daily*、日本 IP、APN plus.4g */
export function softbankNativeDailyKeyFeatures() {
  return pack(
    [
      "本方案為 **SoftBank 原生** 每日型 eSIM，走 SoftBank 4G／5G、**日本 IP**，APN 為 **plus.4g**，延遲較接近在地體驗。",
      "**為什麼選擇 SoftBank 原生每日型？**",
      "**日本原生 IP**：當地出口，造訪日本在地服務、導航與社群通常更順。",
      "**每日高速額度**：可選每日 1GB／2GB／3GB／5GB／10GB；高速用完後一般降速至約 128 kbps，隔日重置。",
      "**APN 提醒**：iPhone 多半自動設定；**Android 手機通常需另外手動設定 APN：plus.4g**。",
      "**支援熱點與常用 App**：熱點分享；選品標示支援 ChatGPT、TikTok、Gemini。",
      "**安裝提醒**：建議抵達日本、於 SoftBank 覆蓋範圍內再安裝／啟用。",
    ],
    "高速額度內：熱門城市測速常見可到數十 Mbps。用完後約 128kbps 等級，僅適合傳訊。日本 IP 對在地 App／網站通常較順。Android 若未設 plus.4g 可能有訊號但無法上網——請先確認 APN。僅供參考。",
  );
}

/** IIJ Docomo 原生：LocalIIJ Daily3GB、日本 IP、APN vmobile.jp */
export function iijDocomoDailyKeyFeatures() {
  return pack(
    [
      "本方案為 **IIJ Docomo（DOCOMO 網路）原生** 每日型，走 Docomo 4G／LTE、**日本 IP**，數據量為**每日 3GB**，適合偏好 Docomo 覆蓋的旅客。",
      "**為什麼選擇 IIJ Docomo 每日型？**",
      "**日本原生 IP**：當地出口，延遲低、連線接近在地用戶體驗。",
      "**每日 3GB 高速**：高速用完後降速至約 256 kbps，隔日重置，可持續使用。",
      "**需手動設定 APN**：安裝後請設定 APN 為 **vmobile.jp**（購買流程會再提醒）。",
      "**支援熱點**：可熱點分享；實際可用 App 依當下網路與設定而定。",
      "**安裝提醒**：建議抵達日本後再安裝；完成 APN 設定後再啟用數據。",
    ],
    "高速額度內：都會區測速常見可到數十 Mbps。用完後約 256kbps，傳訊／輕量網頁通常可用，影音偏卡。若未手動設定 vmobile.jp，常見狀況是有訊號但無法上網。僅供參考。",
  );
}
...
export function japanDailyKeyFeaturesByCarrier() {
  return {
    ...
    [JP_DAILY_TELECOM_SOFTBANK]: softbankNativeDailyKeyFeatures(),
    SoftBank: softbankNativeDailyKeyFeatures(),
