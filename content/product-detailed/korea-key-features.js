...
export function koreaSkNativeUnlimitedKeyFeatures() {
  return pack(
    [
      ...
      "**支援熱點與常用 App**：熱點分享；支援 ChatGPT、TikTok。",
...
/** 每日型／總量型｜SK電信 5G（香港 IP；總量型商品變體為 SKT 單網） */
export function koreaSkt5gKeyFeatures({ kind = "daily" } = {}) {
  const isDaily = kind === "daily";
  return pack(
    [
      isDaily
        ? `本方案為 **SK電信（SKT）5G** 每日型 eSIM（漫遊線路），出網為**香港 IP**，走 SKT 單網。`
        : `本方案為 **SK電信 5G** 總量型 eSIM（漫遊線路），出網為**香港 IP**、APN **cmhk**，走 **SKT 單網**；流量規則為 **用完斷網**。`,
      "**為什麼選擇此方案？**",
      "**SKT 單網**：韓國最大電信之一，熱門城市 5G／4G 覆蓋佳。",
      isDaily
        ? "**每日高速額度**：可選每日 500MB／1GB／2GB；用完後降速至約 384kbps 可持續使用（每日重置）。"
        : "**總量高速流量**：可選 1GB～50GB；**流量用完即斷網**。本店此變體標示不支援熱點。",
      "**安裝提醒**：APN 多為 cmhk；建議在台灣機場先安裝設定好，抵達韓國再啟用。",
      isDaily
        ? "**適合誰**：偏好此線路、接受香港 IP。"
        : "**適合誰**：已精準預估用量、可接受用完斷網；若需雙網或用完續航，請改選雙切換總量型（用完約 128kbps）。",
    ],
    isDaily
      ? "高速額度內：都會區測速常見可到數十 Mbps。用完後約 384kbps，傳訊／輕量網頁通常仍可用，影音會吃力。僅供參考。"
      : "總量高速內：都會區測速常見可到數十 Mbps。流量歸零後會斷網，無法再傳訊——建議預留餘量或改選雙切換（用完降速）。僅供參考。",
  );
}
